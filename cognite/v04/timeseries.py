...
def _get_datapoints_helper(tag_id, aggregates=None, granularity=None, start=None, end=None, **kwargs):
    ...
    url = config.get_base_url(api_version=0.4) + '/projects/{}/timeseries/data/{}'.format(project, quote(tag_id, safe=''))
...
def post_multi_tag_datapoints(timeseries_with_datapoints: List[TimeseriesWithDatapoints], **kwargs):
    ...
    for bin in timeseries_to_upload_binned:
        body = {
            'items': [{"tagId": ts_with_data.tagId, "datapoints": [dp.__dict__ for dp in ts_with_data.datapoints]} for
                      ts_with_data in bin]
        }
...
def get_latest(tag_id, **kwargs):
    ...
    '''
    api_key, project = config.get_config_variables(kwargs.get('api_key'), kwargs.get('project'))
    url = config.get_base_url(api_version=0.4) + '/projects/{}/timeseries/latest/{}'.format(project, quote(tag_id, safe=''))
    headers = {
        'api-key': api_key,
        'accept': 'application/json'
    }
