...
from cognite.client.data_classes import Function, FunctionCall, FunctionCallList, FunctionCallLog, FunctionFilter, FunctionList, FunctionSchedule, FunctionSchedulesFilter, FunctionSchedulesList, FunctionsLimits, TimestampRange
...
class FunctionsAPI(APIClient):
    ...
    def create(self, name, folder=None, file_id=None, function_path=HANDLER_FILE_NAME, function_handle=None, external_id=None, description='', owner='', api_key=None, secrets=None, env_vars=None, cpu=None, memory=None, runtime=None, metadata=None, index_url=None, extra_index_urls=None):
        ...
        url = '/functions'
        function: Dict[(str, Any)] = {'name': name, 'description': description, 'owner': owner, 'fileId': file_id, 'functionPath': function_path, 'envVars': env_vars, 'metadata': metadata}
        if cpu:
            function['cpu'] = cpu
        if memory:
    ...
    def list(self, name=None, owner=None, file_id=None, status=None, external_id_prefix=None, created_time=None, limit=LIST_LIMIT_DEFAULT):
        if is_unlimited(limit):
            limit = LIST_LIMIT_CEILING
        filter = FunctionFilter(name=name, owner=owner, file_id=file_id, status=status, external_id_prefix=external_id_prefix, created_time=created_time).dump(camel_case=True)
    ...
    def retrieve_multiple(self, ids=None, external_ids=None):
        ...
        return self._retrieve_multiple(identifiers=IdentifierSequence.load(ids=ids, external_ids=external_ids), resource_cls=Function, list_cls=FunctionList)
    ...
    def _zip_and_upload_folder(self, folder, name, external_id=None):
        ...
        try:
            with TemporaryDirectory() as tmpdir:
                ...
                file = cast(FileMetadata, self._cognite_client.files.upload(zip_path, name=f'{name}.zip', external_id=external_id, overwrite=overwrite))
...
''')
    ...
    def _assert_exactly_one_of_folder_or_file_id_or_function_handle(folder, file_id, function_handle):
        ...
        if (len(given_source_code_options) < 1):
            ...
            raise TypeError((('Exactly one of the arguments folder, file_id and handle is required, but ' + ', '.join(given_source_code_options)) + ' were given.'))
...
def _validate_function_handle(function_handle):
    ...
    if (not set(function_handle.__code__.co_varnames[:function_handle.__code__.co_argcount]).issubset({'data', 'client', 'secrets', 'function_call_info'})):
        raise TypeError('Arguments to function referenced by function_handle must be a subset of (data, client, secrets, function_call_info)')
...
class FunctionCallsAPI(APIClient):
    ...
    def list(self, function_id=None, function_external_id=None, status=None, schedule_id=None, start_time=None, end_time=None, limit=LIST_LIMIT_DEFAULT):
        ...
        filter = FunctionCallsFilter(status=status, schedule_id=schedule_id, start_time=start_time, end_time=end_time).dump(camel_case=True)
        resource_path = f'/functions/{function_id}/calls'
        return self._list(method='POST', resource_path=resource_path, filter=filter, limit=limit, resource_cls=FunctionCall, list_cls=FunctionCallList)
    ...
    def retrieve(self, call_id, function_id=None, function_external_id=None):
        ...
        return self._retrieve_multiple(resource_path=resource_path, identifiers=identifiers, resource_cls=FunctionCall, list_cls=FunctionCallList)
...
class FunctionSchedulesAPI(APIClient):
    ...
    def retrieve(self, id):
        return self._retrieve_multiple(identifiers=IdentifierSequence.load(ids=id), resource_cls=FunctionSchedule, list_cls=FunctionSchedulesList)

    def list(self, name=None, function_id=None, function_external_id=None, created_time=None, cron_expression=None, limit=LIST_LIMIT_DEFAULT):
        ...
        if is_unlimited(limit):
            limit = LIST_LIMIT_CEILING
        filter = FunctionSchedulesFilter(name=name, function_id=function_id, function_external_id=function_external_id, created_time=created_time, cron_expression=cron_expression).dump(camel_case=True)
        res = self._post(url_path=f'{self._RESOURCE_PATH}/list', json={'filter': filter, 'limit': limit})
        return self._LIST_CLASS._load(res.json()['items'], cognite_client=self._cognite_client)

    def create(self, name, cron_expression, function_id=None, function_external_id=None, client_credentials=None, description='', data=None):
        ...
        body: Dict[(str, List[Dict[(str, Union[(str, int, None, Dict)])]])] = {'items': [{'name': name, 'description': description, 'functionId': function_id, 'functionExternalId': function_external_id, 'cronExpression': cron_expression, 'nonce': nonce}]}
        if data:
