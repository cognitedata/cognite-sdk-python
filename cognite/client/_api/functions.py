...
class FunctionsAPI(APIClient):
    ...
    def create(
        ...
        Args:
            ...
            cpu (Number | None): Number of CPU cores per function. Allowed range and default are given in the `limits endpoint. <https://developer.cognite.com/api#tag/Functions/operation/functionsLimits>`_, and None translates to the API default. On Azure, only the default value is used.
            memory (Number | None): Memory per function measured in GB. Allowed range and default are given in the `limits endpoint. <https://developer.cognite.com/api#tag/Functions/operation/functionsLimits>`_, and None translates to the API default. On Azure, only the default value is used.
