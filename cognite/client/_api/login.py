...
class LoginAPI(APIClient):
    ...
    def status(self):
        if not isinstance(self._config.credentials, APIKey):
            warnings.warn(
                "It seems you are trying to reach API endpoint `/login/status` which is only valid when authenticating using an API key - without an API key. Try `client.iam.token.inspect` instead",
                UserWarning,
                stacklevel=2,
            )
