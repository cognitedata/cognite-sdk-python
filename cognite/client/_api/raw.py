...
class RawDatabasesAPI(APIClient):
    ...
    def __call__(self, chunk_size=None, limit=None):
        return self._list_generator(list_cls=DatabaseList, resource_cls=Database, chunk_size=chunk_size, method='GET', limit=limit)
    ...
    def delete(self, name, recursive=False):
        ...
        tasks = [{'url_path': (self._RESOURCE_PATH + '/delete'), 'json': {'items': chunk, 'recursive': recursive}} for chunk in chunks]
        summary = utils._concurrency.execute_tasks(self._post, tasks, max_workers=self._config.max_workers)
        summary.raise_compound_exception_if_failed_tasks(task_unwrap_fn=(lambda task: task['json']['items']), task_list_element_unwrap_fn=(lambda el: el['name']))
...
class RawTablesAPI(APIClient):
    ...
    def __call__(self, db_name, chunk_size=None, limit=None):
        for tb in self._list_generator(list_cls=TableList, resource_cls=Table, resource_path=utils._auxiliary.interpolate_and_url_encode(self._RESOURCE_PATH, db_name), chunk_size=chunk_size, method='GET', limit=limit):
    ...
    def create(self, db_name, name):
        ...
        if isinstance(name, str):
            ...
            items = [{'name': n} for n in name]
        tb = self._create_multiple(list_cls=TableList, resource_cls=Table, resource_path=utils._auxiliary.interpolate_and_url_encode(self._RESOURCE_PATH, db_name), items=items)
    ...
    def delete(self, db_name, name):
        ...
        items = [{'name': n} for n in name]
        chunks = utils._auxiliary.split_into_chunks(items, self._DELETE_LIMIT)
        tasks = [{'url_path': (utils._auxiliary.interpolate_and_url_encode(self._RESOURCE_PATH, db_name) + '/delete'), 'json': {'items': chunk}} for chunk in chunks]
        summary = utils._concurrency.execute_tasks(self._post, tasks, max_workers=self._config.max_workers)
        summary.raise_compound_exception_if_failed_tasks(task_unwrap_fn=(lambda task: task['json']['items']), task_list_element_unwrap_fn=(lambda el: el['name']))

    def list(self, db_name, limit=25):
        tb = self._list(list_cls=TableList, resource_cls=Table, resource_path=utils._auxiliary.interpolate_and_url_encode(self._RESOURCE_PATH, db_name), method='GET', limit=limit)
...
class RawRowsAPI(APIClient):
    ...
    def __call__(self, db_name, table_name, chunk_size=None, limit=None, min_last_updated_time=None, max_last_updated_time=None, columns=None):
        return self._list_generator(list_cls=RowList, resource_cls=Row, resource_path=utils._auxiliary.interpolate_and_url_encode(self._RESOURCE_PATH, db_name, table_name), chunk_size=chunk_size, method='GET', limit=limit, filter={'minLastUpdatedTime': min_last_updated_time, 'maxLastUpdatedTime': max_last_updated_time, 'columns': self._make_columns_param(columns)})

    def insert(self, db_name, table_name, row, ensure_parent=False):
        chunks = self._process_row_input(row)
        tasks = [{'url_path': utils._auxiliary.interpolate_and_url_encode(self._RESOURCE_PATH, db_name, table_name), 'json': {'items': chunk}, 'params': {'ensureParent': ensure_parent}} for chunk in chunks]
        summary = utils._concurrency.execute_tasks(self._post, tasks, max_workers=self._config.max_workers)
        summary.raise_compound_exception_if_failed_tasks(task_unwrap_fn=(lambda task: task['json']['items']), task_list_element_unwrap_fn=(lambda row: row.get('key')))
    ...
    def delete(self, db_name, table_name, key):
        ...
        chunks = utils._auxiliary.split_into_chunks(items, self._DELETE_LIMIT)
        tasks = [{'url_path': (utils._auxiliary.interpolate_and_url_encode(self._RESOURCE_PATH, db_name, table_name) + '/delete'), 'json': {'items': chunk}} for chunk in chunks]
        summary = utils._concurrency.execute_tasks(self._post, tasks, max_workers=self._config.max_workers)
        summary.raise_compound_exception_if_failed_tasks(task_unwrap_fn=(lambda task: task['json']['items']), task_list_element_unwrap_fn=(lambda el: el['key']))

    def retrieve(self, db_name, table_name, key):
        return self._retrieve(cls=Row, resource_path=utils._auxiliary.interpolate_and_url_encode(self._RESOURCE_PATH, db_name, table_name), identifier=Identifier(key))

    def list(self, db_name, table_name, min_last_updated_time=None, max_last_updated_time=None, columns=None, limit=25):
        if is_unlimited(limit):
            cursors = self._get(url_path=utils._auxiliary.interpolate_and_url_encode('/raw/dbs/{}/tables/{}/cursors', db_name, table_name), params={'minLastUpdatedTime': min_last_updated_time, 'maxLastUpdatedTime': max_last_updated_time, 'numberOfCursors': self._config.max_workers}).json()['items']
        else:
            cursors = [None]
        tasks = [dict(list_cls=RowList, resource_cls=Row, resource_path=utils._auxiliary.interpolate_and_url_encode(self._RESOURCE_PATH, db_name, table_name), method='GET', filter={'columns': self._make_columns_param(columns), 'minLastUpdatedTime': min_last_updated_time, 'maxLastUpdatedTime': max_last_updated_time}, limit=limit, initial_cursor=cursor) for cursor in cursors]
    ...
    def retrieve_dataframe(self, db_name, table_name, min_last_updated_time=None, max_last_updated_time=None, columns=None, limit=25):
