...
class LabelsAPI(APIClient):
    ...
    def __call__(self, name=None, external_id_prefix=None, limit=None, chunk_size=None, data_set_ids=None, data_set_external_ids=None):
        ...
        filter = LabelDefinitionFilter(name=name, external_id_prefix=external_id_prefix, data_set_ids=data_set_ids_processed).dump(camel_case=True)
        return self._list_generator(list_cls=LabelDefinitionList, resource_cls=LabelDefinition, method='POST', limit=limit, filter=filter, chunk_size=chunk_size)
    ...
    def list(self, name=None, external_id_prefix=None, data_set_ids=None, data_set_external_ids=None, limit=25):
        ...
        if (data_set_ids or data_set_external_ids):
            data_set_ids_processed = IdentifierSequence.load(data_set_ids, data_set_external_ids).as_dicts()
        filter = LabelDefinitionFilter(name=name, external_id_prefix=external_id_prefix, data_set_ids=data_set_ids_processed).dump(camel_case=True)
        return self._list(list_cls=LabelDefinitionList, resource_cls=LabelDefinition, method='POST', limit=limit, filter=filter)
