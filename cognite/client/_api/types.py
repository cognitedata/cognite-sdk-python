...
class TypesAPI(APIClient):
    ...
    def __call__(
        ...
        limit: int = None,
    ) -> Generator[Union[Type, TypeList], None, None]:
        """Iterate over type definitions

        Fetches type definitions as they are iterated over, so you keep a limited number of type definitions in memory.

        Args:
            chunk_size (int, optional): Number of Type objects to return in each chunk. Defaults to yielding one Type a time.
            ...
            limit (int, optional): Maximum number of type definitions to return. Defaults to 25. Set to -1, float("inf") or None
                to return all items.

        Yields:
            Union[Type, TypeList]: yields Type one by one if chunk is not specified, else TypeList objects.
        """
