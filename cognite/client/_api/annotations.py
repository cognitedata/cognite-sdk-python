...
class AnnotationsAPI(APIClient):
    ...
    def create(self, annotations):
        ...
        return self._create_multiple(list_cls=AnnotationList, resource_cls=Annotation, resource_path=(self._RESOURCE_PATH + '/'), items=annotations)
    ...
    def suggest(self, annotations):
        assert_type(annotations, 'annotations', [Annotation, Sequence])
        items: Union[(List[Dict[(str, Any)]], Dict[(str, Any)])] = ([self._sanitize_suggest_item(ann) for ann in annotations] if isinstance(annotations, Sequence) else self._sanitize_suggest_item(annotations))
        return self._create_multiple(list_cls=AnnotationList, resource_cls=Annotation, resource_path=(self._RESOURCE_PATH + '/suggest'), items=items)
    ...
    def list(self, filter, limit=25):
        ...
        if ('annotatedResourceIds' in filter):
            filter['annotatedResourceIds'] = [{to_camel_case(k): v for (k, v) in f.items()} for f in filter['annotatedResourceIds']]
    ...
    def update(self, item):
        return self._update_multiple(list_cls=AnnotationList, resource_cls=Annotation, update_cls=AnnotationUpdate, items=item)
