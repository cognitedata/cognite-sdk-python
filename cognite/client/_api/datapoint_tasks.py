...
from cognite.client.utils._auxiliary import convert_all_keys_to_snake_case, import_legacy_protobuf, is_unlimited, to_camel_case, to_snake_case
from cognite.client.utils._identifier import Identifier
from cognite.client.utils._time import align_start_and_end_for_granularity, granularity_to_ms, split_time_range, time_ago_to_ms, timestamp_to_ms
if (not import_legacy_protobuf()):
    ...
    from cognite.client._proto_legacy.data_point_list_response_pb2 import DataPointListItem
...
class _DatapointsQuery():
    ...
    def is_single_identifier(self):
        return ((isinstance(self.id, (dict, numbers.Integral)) and (self.external_id is None)) or (isinstance(self.external_id, (dict, str)) and (self.id is None)))

class _SingleTSQueryValidator():

    def __init__(self, user_query):
        self.user_query = user_query
        self.defaults: Dict[(str, Any)] = dict(start=user_query.start, end=user_query.end, limit=user_query.limit, aggregates=user_query.aggregates, granularity=user_query.granularity, include_outside_points=user_query.include_outside_points, ignore_unknown_ids=user_query.ignore_unknown_ids)
    ...
    def _validate_id_or_xid(self, id_or_xid, arg_name, exp_type):
        ...
        for ts in id_or_xid_seq:
            if isinstance(ts, exp_type):
                ...
                queries.append(self._validate_and_create_query(cast(Union[(DatapointsQueryId, DatapointsQueryExternalId)], ts_dct)))
    ...
    def _raise_on_wrong_ts_identifier_type(id_or_xid, arg_name, exp_type):
        raise TypeError(f'Got unsupported type {type(id_or_xid)}, as, or part of argument `{arg_name}`. Expected one of {exp_type}, {dict} or a (mixed) list of these, but got `{id_or_xid}`.')
    ...
    def _validate_user_supplied_dict_keys(dct, arg_name):
        ...
        opt_dct_keys = {'start', 'end', 'aggregates', 'granularity', 'limit', 'include_outside_points', 'ignore_unknown_ids'}
        bad_keys = ((set(dct) - opt_dct_keys) - {arg_name})
        if (not bad_keys):
            return dct
        raise KeyError(f'Dict provided by argument `{arg_name}` included key(s) not understood: {sorted(bad_keys)}. Required key: `{arg_name}`. Optional: {list(opt_dct_keys)}.')
    ...
    def _convert_parameters(self, dct, limit, is_raw):
        identifier = Identifier.of_either(cast(Optional[int], dct.get('id')), cast(Optional[str], dct.get('external_id')))
        (start, end) = self._verify_time_range(dct['start'], dct['end'], dct['granularity'], is_raw, identifier)
        converted = {'identifier': identifier, 'start': start, 'end': end, 'limit': limit, 'ignore_unknown_ids': dct['ignore_unknown_ids']}
        if is_raw:
    ...
    def _verify_limit(self, limit):
        ...
        raise TypeError(f'Parameter `limit` must be a non-negative integer -OR- one of [None, -1, inf] to indicate an unlimited query. Got: {limit} with type: {type(limit)}')
    ...
    def _verify_time_range(self, start, end, granularity, is_raw, identifier):
        ...
        if (end <= start):
            raise ValueError(f'Invalid time range, end={end} must be later than start={start} (from query: {identifier.as_dict(camel_case=False)})')
        if (not is_raw):
...
class _SingleTSQueryBase():
    ...
    def __init__(self, *, identifier: Identifier, start: int, end: int, max_query_limit: int, limit: Optional[int], include_outside_points: bool, ignore_unknown_ids: bool):
        ...
        if (self.include_outside_points and (self.limit is not None)):
            warnings.warn("When using `include_outside_points=True` with a finite `limit` you may get a large gap between the last 'inside datapoint' and the 'after/outside' datapoint. Note also that the up-to-two outside points come in addition to your given `limit`; asking for 5 datapoints might yield 5, 6 or 7. It's a feature, not a bug ;)", UserWarning)
...
class _SingleTSQueryRaw(_SingleTSQueryBase):
    ...
    def to_payload(self):
        return {**self.identifier.as_dict(), 'start': self.start, 'end': self.end, 'limit': self.capped_limit, 'includeOutsidePoints': self.include_outside_points}
...
class _SingleTSQueryAgg(_SingleTSQueryBase):
    ...
    def to_payload(self):
        return {**self.identifier.as_dict(), 'start': self.start, 'end': self.end, 'aggregates': self.aggregates_cc, 'granularity': self.granularity, 'limit': self.capped_limit, 'includeOutsidePoints': self.include_outside_points}
...
def get_ts_info_from_proto(res):
    return {'id': res.id, 'external_id': res.externalId, 'is_string': res.isString, 'is_step': res.isStep, 'unit': res.unit}
...
class OutsideDpsFetchSubtask(BaseDpsFetchSubtask):
    ...
    def _create_payload_item(self):
        return CustomDatapoints({**self.identifier.as_dict(), 'start': self.start, 'end': self.end, 'limit': 0, 'includeOutsidePoints': True})
...
class SerialFetchSubtask(BaseDpsFetchSubtask):
    ...
    def __init__(self, *, limit: Optional[int], aggregates: Optional[List[str]], granularity: Optional[str], subtask_idx: Tuple[(float, ...)], **kwargs: Any):
    ...
    def _create_payload_item(self, remaining_limit):
        return CustomDatapoints({**self.identifier.as_dict(), 'start': self.next_start, 'end': self.end, 'limit': min(remaining_limit, self.max_query_limit), **self.agg_kwargs})
...
class SplittingFetchSubtask(SerialFetchSubtask):
    ...
    def _split_self_into_new_subtasks_if_needed(self, last_ts):
        ...
        static_params: Dict[(str, Any)] = {'parent': self.parent, 'priority': self.priority, 'identifier': self.identifier, 'aggregates': self.aggregates, 'granularity': self.granularity, 'max_query_limit': self.max_query_limit, 'is_raw_query': self.is_raw_query}
        split_idxs = self._create_subtasks_idxs(n_new_tasks)
        new_subtasks = [SplittingFetchSubtask(start=start, end=end, limit=remaining_limit, subtask_idx=idx, **static_params) for (start, end, idx) in zip(boundaries[1:(- 1)], boundaries[2:], split_idxs)]
...
class BaseConcurrentTask():
    ...
    def split_into_subtasks(self, max_workers, n_tot_queries):
        ...
        if (self.eager_mode and self.query.include_outside_points):
            self.subtask_outside_points = OutsideDpsFetchSubtask(start=self.query.start, end=self.query.end, identifier=self.query.identifier, parent=self)
    ...
    def _create_uniformly_split_subtasks(self, n_workers_per_queries):
        ...
        return [SplittingFetchSubtask(start=start, end=end, limit=limit, subtask_idx=(i,), parent=self, priority=((i - 1) if self.has_limit else 0), identifier=self.query.identifier, aggregates=self.query.aggregates_cc, granularity=self.query.granularity, max_query_limit=self.query.max_query_limit, is_raw_query=self.query.is_raw_query) for (i, (start, end)) in enumerate(zip(boundaries[:(- 1)], boundaries[1:]), 1)]
...
class BaseConcurrentRawTask(BaseConcurrentTask):
    ...
    def _create_empty_result(self):
        ...
        return DatapointsArray._load({**self.ts_info_dct, 'timestamp': np.array([], dtype=np.int64), 'value': np.array([], dtype=self.raw_dtype)})
    ...
    def get_result(self):
        ...
        if self.use_numpy:
            return DatapointsArray._load({**self.ts_info_dct, 'timestamp': create_array_from_dps_container(self.ts_data), 'value': create_array_from_dps_container(self.dps_data)})
        return Datapoints(**self.ts_info_dct, timestamp=create_list_from_dps_container(self.ts_data), value=create_list_from_dps_container(self.dps_data))
    ...
    def _unpack_and_store_numpy(self, idx, dps):
        ...
        if self.has_non_count_aggs:
            try:
                ...
                arr = np.array([tuple((getattr(dp, agg, math.nan) for agg in self.float_aggs)) for dp in dps], dtype=np.float64)
