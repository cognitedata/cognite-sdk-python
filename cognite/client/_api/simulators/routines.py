...
class SimulatorRoutinesAPI(APIClient):
    ...
    def __call__(
        ...
        Returns:
            Iterator[SimulatorRoutine] | Iterator[SimulatorRoutineList]: yields Simulator one by one if chunk is not specified, else SimulatorList objects.
        """
    ...
    def create(
        ...
        Examples:
            Create new simulator routines:
                >>> from cognite.client import CogniteClient
                >>> from cognite.client.data_classes import SimulatorRoutineWrite
                >>> client = CogniteClient()
                >>> routine = [SimulatorRoutineWrite(name="simulator-routine")]
                >>> res = client.simulators.routines.create(routine)
        """
