...
from cognite.client.data_classes import APIKey, APIKeyList, ClientCredentials, CreatedSession, Group, GroupList, SecurityCategory, SecurityCategoryList, ServiceAccount, ServiceAccountList, Session, SessionList
...
class APIKeysAPI(APIClient):
    ...
    def list(self, include_deleted=False, all=False, service_account_id=None):
        res = self._get(self._RESOURCE_PATH, params={'all': all, 'serviceAccountId': service_account_id, 'includeDeleted': include_deleted})
...
class SecurityCategoriesAPI(APIClient):
    ...
    def create(self, security_category):
        return self._create_multiple(list_cls=SecurityCategoryList, resource_cls=SecurityCategory, items=security_category)
...
class TokenAPI(APIClient):
    ...
    def inspect(self):
        if isinstance(self._config.credentials, APIKey):
            warnings.warn('It seems you are trying to reach API endpoint `/token/inspect` which is not valid when authenticating using an API key. Try `client.login.status` instead', UserWarning, stacklevel=2)
