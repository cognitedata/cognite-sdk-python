...
from cognite.client.data_classes import ExtractionPipeline, ExtractionPipelineConfig, ExtractionPipelineConfigRevisionList, ExtractionPipelineList, ExtractionPipelineRun, ExtractionPipelineRunFilter, ExtractionPipelineRunList, ExtractionPipelineUpdate, TimestampRange
...
class ExtractionPipelinesAPI(APIClient):
    ...
    def retrieve(self, id=None, external_id=None):
        ...
        return self._retrieve_multiple(list_cls=ExtractionPipelineList, resource_cls=ExtractionPipeline, identifiers=identifiers)

    def retrieve_multiple(self, ids=None, external_ids=None, ignore_unknown_ids=False):
        identifiers = IdentifierSequence.load(ids=ids, external_ids=external_ids)
        return self._retrieve_multiple(list_cls=ExtractionPipelineList, resource_cls=ExtractionPipeline, identifiers=identifiers, ignore_unknown_ids=ignore_unknown_ids)
    ...
    def create(self, extraction_pipeline=None, **kwargs: Dict[(str, Any)]):
        extraction_pipeline = cast(Union[(ExtractionPipeline, Sequence[ExtractionPipeline])], handle_deprecated_camel_case_argument(extraction_pipeline, 'extractionPipeline', 'create', kwargs))
        utils._auxiliary.assert_type(extraction_pipeline, 'extraction_pipeline', [ExtractionPipeline, Sequence])
        return self._create_multiple(list_cls=ExtractionPipelineList, resource_cls=ExtractionPipeline, items=extraction_pipeline)
    ...
    def update(self, item):
        return self._update_multiple(list_cls=ExtractionPipelineList, resource_cls=ExtractionPipeline, update_cls=ExtractionPipelineUpdate, items=item)
...
class ExtractionPipelineRunsAPI(APIClient):
    ...
    def list(self, external_id, statuses=None, message_substring=None, created_time=None, limit=25):
        if ((statuses is not None) or (message_substring is not None) or (created_time is not None)):
            filter = ExtractionPipelineRunFilter(external_id=external_id, statuses=statuses, message=StringFilter(substring=message_substring), created_time=created_time).dump(camel_case=True)
            return self._list(list_cls=ExtractionPipelineRunList, resource_cls=ExtractionPipelineRun, method='POST', limit=limit, filter=filter)
        return self._list(list_cls=ExtractionPipelineRunList, resource_cls=ExtractionPipelineRun, method='GET', limit=limit, filter={'externalId': external_id})
...
class ExtractionPipelineConfigsAPI(APIClient):
    ...
    def retrieve(self, external_id, revision=None, active_at_time=None):
        response = self._get('/extpipes/config', params={'externalId': external_id, 'activeAtTime': active_at_time, 'revision': revision})
