...
from cognite.client.data_classes import Asset, AssetAggregate, AssetFilter, AssetList, AssetUpdate, GeoLocationFilter, LabelFilter, TimestampRange
...
class AssetsAPI(APIClient):
    ...
    def __call__(self, chunk_size=None, name=None, parent_ids=None, parent_external_ids=None, asset_subtree_ids=None, asset_subtree_external_ids=None, metadata=None, data_set_ids=None, data_set_external_ids=None, labels=None, geo_location=None, source=None, created_time=None, last_updated_time=None, root=None, external_id_prefix=None, aggregated_properties=None, limit=None, partitions=None):
        ...
        filter = AssetFilter(name=name, parent_ids=parent_ids, parent_external_ids=parent_external_ids, asset_subtree_ids=asset_subtree_ids_processed, data_set_ids=data_set_ids_processed, labels=labels, geo_location=geo_location, metadata=metadata, source=source, created_time=created_time, last_updated_time=last_updated_time, root=root, external_id_prefix=external_id_prefix).dump(camel_case=True)
        return self._list_generator(list_cls=AssetList, resource_cls=Asset, method='POST', chunk_size=chunk_size, filter=filter, limit=limit, partitions=partitions, other_params=({'aggregatedProperties': aggregated_properties} if aggregated_properties else {}))
    ...
    def retrieve_multiple(self, ids=None, external_ids=None, ignore_unknown_ids=False):
        ...
        return self._retrieve_multiple(list_cls=AssetList, resource_cls=Asset, identifiers=identifiers, ignore_unknown_ids=ignore_unknown_ids)

    def list(self, name=None, parent_ids=None, parent_external_ids=None, asset_subtree_ids=None, asset_subtree_external_ids=None, data_set_ids=None, data_set_external_ids=None, labels=None, geo_location=None, metadata=None, source=None, created_time=None, last_updated_time=None, root=None, external_id_prefix=None, aggregated_properties=None, partitions=None, limit=25):
        if aggregated_properties:
            aggregated_properties = [utils._auxiliary.to_camel_case(s) for s in aggregated_properties]
        asset_subtree_ids_processed = None
        if (asset_subtree_ids or asset_subtree_external_ids):
        ...
        if (data_set_ids or data_set_external_ids):
            data_set_ids_processed = IdentifierSequence.load(data_set_ids, data_set_external_ids).as_dicts()
        filter = AssetFilter(name=name, parent_ids=parent_ids, parent_external_ids=parent_external_ids, asset_subtree_ids=asset_subtree_ids_processed, data_set_ids=data_set_ids_processed, labels=labels, geo_location=geo_location, metadata=metadata, source=source, created_time=created_time, last_updated_time=last_updated_time, root=root, external_id_prefix=external_id_prefix).dump(camel_case=True)
        return self._list(list_cls=AssetList, resource_cls=Asset, method='POST', limit=limit, filter=filter, other_params=({'aggregatedProperties': aggregated_properties} if aggregated_properties else {}), partitions=partitions)
    ...
    def delete(self, id=None, external_id=None, recursive=False, ignore_unknown_ids=False):
        self._delete_multiple(identifiers=IdentifierSequence.load(ids=id, external_ids=external_id), wrap_ids=True, extra_body_fields={'recursive': recursive, 'ignoreUnknownIds': ignore_unknown_ids})
    ...
    def search(self, name=None, description=None, query=None, filter=None, limit=100):
        return self._search(list_cls=AssetList, search={'name': name, 'description': description, 'query': query}, filter=(filter or {}), limit=limit)
...
class _AssetPoster():
    ...
    def __init__(self, assets, client):
        ...
        self.external_id_to_children: Dict[(str, Set[Asset])] = {external_id: set() for external_id in self.remaining_external_ids}
    ...
    def assets_remaining(self):
        return (((len(self.posted_assets) + len(self.may_have_been_posted_assets)) + len(self.not_posted_assets)) < self.num_of_assets)
        ...
        for asset in assets:
            ...
            if parent_ref:
                if (asset.parent_id is not None):
                    raise AssertionError(f"An asset has both parent_id '{asset.parent_id}' and parent_external_id '{asset.parent_external_id}' set.")
    ...
    def _initialize_asset_to_descendant_count(self, asset):
        for child in self.external_id_to_children[cast(str, asset.external_id)]:
            self.external_id_to_descendent_count[cast(str, asset.external_id)] += (1 + self._initialize_asset_to_descendant_count(child))
        ...
        for external_id in self.remaining_external_ids:
            ...
            if (external_id in self.remaining_external_ids_set):
                ...
                if (is_root or has_parent_id or is_unblocked):
                    unblocked_assets_chunk.update(self._get_assets_unblocked_locally(asset, (limit - len(unblocked_assets_chunk))))
