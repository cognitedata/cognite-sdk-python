...
from cognite.client.data_classes.templates import GraphQlResponse, TemplateGroup, TemplateGroupList, TemplateGroupVersion, TemplateGroupVersionList, TemplateInstance, TemplateInstanceList, TemplateInstanceUpdate, View, ViewList, ViewResolveItem, ViewResolveList
...
class TemplateGroupsAPI(APIClient):
    ...
    def upsert(self, template_groups):
        ...
        updated = self._post(path, {'items': [item.dump(camel_case=True) for item in template_groups_processed]}).json()['items']
    ...
    def retrieve_multiple(self, external_ids, ignore_unknown_ids=False):
        ...
        return self._retrieve_multiple(list_cls=TemplateGroupList, resource_cls=TemplateGroup, identifiers=identifiers, ignore_unknown_ids=ignore_unknown_ids)

    def list(self, limit=25, owners=None):
        filter = {}
        if (owners is not None):
            filter['owners'] = owners
        return self._list(list_cls=TemplateGroupList, resource_cls=TemplateGroup, method='POST', limit=limit, filter=filter, partitions=None, sort=None)

    def delete(self, external_ids, ignore_unknown_ids=False):
        return self._delete_multiple(wrap_ids=True, identifiers=IdentifierSequence.load(external_ids=external_ids), extra_body_fields={'ignoreUnknownIds': ignore_unknown_ids})
...
class TemplateGroupVersionsAPI(APIClient):
    ...
    def list(self, external_id, limit=25, min_version=None, max_version=None):
        ...
        return self._list(list_cls=TemplateGroupVersionList, resource_cls=TemplateGroupVersion, resource_path=resource_path, method='POST', limit=limit, filter=filter)
...
class TemplateInstancesAPI(APIClient):
    ...
    def create(self, external_id, version, instances):
        ...
        return self._create_multiple(list_cls=TemplateInstanceList, resource_cls=TemplateInstance, resource_path=resource_path, items=instances)

    def upsert(self, external_id, version, instances):
        if isinstance(instances, TemplateInstance):
            instances = [instances]
        resource_path = (utils._auxiliary.interpolate_and_url_encode(self._RESOURCE_PATH, external_id, version) + '/upsert')
        updated = self._post(resource_path, {'items': [instance.dump(camel_case=True) for instance in instances]}).json()['items']
    ...
    def update(self, external_id, version, item):
        resource_path = utils._auxiliary.interpolate_and_url_encode(self._RESOURCE_PATH, external_id, version)
        return self._update_multiple(list_cls=TemplateInstanceList, resource_cls=TemplateInstance, update_cls=TemplateInstanceUpdate, items=item, resource_path=resource_path)

    def retrieve_multiple(self, external_id, version, external_ids, ignore_unknown_ids=False):
        resource_path = utils._auxiliary.interpolate_and_url_encode(self._RESOURCE_PATH, external_id, version)
        identifiers = IdentifierSequence.load(ids=None, external_ids=external_ids)
        return self._retrieve_multiple(list_cls=TemplateInstanceList, resource_cls=TemplateInstance, resource_path=resource_path, identifiers=identifiers, ignore_unknown_ids=ignore_unknown_ids)
    ...
    def list(self, external_id, version, limit=25, data_set_ids=None, template_names=None):
        ...
        return self._list(list_cls=TemplateInstanceList, resource_cls=TemplateInstance, resource_path=resource_path, method='POST', limit=limit, filter=filter)

    def delete(self, external_id, version, external_ids, ignore_unknown_ids=False):
        resource_path = utils._auxiliary.interpolate_and_url_encode(self._RESOURCE_PATH, external_id, version)
        return self._delete_multiple(resource_path=resource_path, identifiers=IdentifierSequence.load(external_ids=external_ids), wrap_ids=True, extra_body_fields={'ignoreUnknownIds': ignore_unknown_ids})
...
class TemplateViewsAPI(APIClient):
    ...
    def resolve(self, external_id, version, view_external_id, input, limit=25):
        ...
        return self._list(list_cls=ViewResolveList, resource_cls=ViewResolveItem, url_path=url_path, method='POST', limit=limit, other_params={'externalId': view_external_id, 'input': input})
    ...
    def delete(self, external_id, version, view_external_id, ignore_unknown_ids=False):
        resource_path = utils._auxiliary.interpolate_and_url_encode(self._RESOURCE_PATH, external_id, version)
        return self._delete_multiple(resource_path=resource_path, identifiers=IdentifierSequence.load(external_ids=view_external_id), wrap_ids=True, extra_body_fields={'ignoreUnknownIds': ignore_unknown_ids})
