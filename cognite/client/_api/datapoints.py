...
from cognite.client._api.datapoint_tasks import DPS_LIMIT, DPS_LIMIT_AGG, BaseConcurrentTask, BaseDpsFetchSubtask, CustomDatapoints, DatapointsPayload, SplittingFetchSubtask, _DatapointsQuery, _SingleTSQueryBase, _SingleTSQueryValidator
from cognite.client._api.synthetic_time_series import SyntheticDatapointsAPI
from cognite.client._api_client import APIClient
from cognite.client.data_classes.datapoints import Datapoints, DatapointsArray, DatapointsArrayList, DatapointsList, LatestDatapointQuery
from cognite.client.exceptions import CogniteAPIError, CogniteNotFoundError
from cognite.client.utils._auxiliary import assert_type, find_duplicates, import_legacy_protobuf, local_import, split_into_chunks, split_into_n_parts, validate_user_input_dict_with_identifier
...
if (not import_legacy_protobuf()):
    ...
    from cognite.client._proto_legacy.data_point_list_response_pb2 import DataPointListItem, DataPointListResponse
if TYPE_CHECKING:
...
class DpsFetchStrategy(ABC):
    ...
    def _finalize_tasks(self, ordered_results, resource_lst):
        return resource_lst([ts_task.get_result() for ts_task in ordered_results], cognite_client=self.dps_client._cognite_client)

    def _make_dps_request_using_protobuf(self, payload):
        return self.dps_client._do_request(json=payload, method='POST', url_path=(self.dps_client._RESOURCE_PATH + '/list'), accept='application/protobuf', timeout=self.dps_client._config.timeout).content
...
class ChunkingDpsFetcher(DpsFetchStrategy):
    ...
    def _fetch_all(self, pool, use_numpy):
        ...
        for future in pool.as_completed(initial_futures_dct):
            ...
            (new_ts_tasks, chunk_missing) = self._create_ts_tasks_and_handle_missing(res_lst, chunk_agg_qs, chunk_raw_qs, initial_query_limits, use_numpy)
        ...
        if ts_tasks_left:
            self._add_to_subtask_pools(chain.from_iterable((task.split_into_subtasks(max_workers=self.max_workers, n_tot_queries=len(ts_tasks_left)) for task in ts_tasks_left)))
    ...
    def _create_ts_tasks_and_handle_missing(self, res, chunk_agg_qs, chunk_raw_qs, initial_query_limits, use_numpy):
        ...
        ts_tasks = {query: query.ts_task_type(query=query, eager_mode=False, use_numpy=use_numpy, first_dps_batch=res, first_limit=initial_query_limits[query]) for (res, query) in zip(res, chain(chunk_agg_qs, chunk_raw_qs))}
...
class DatapointsAPI(APIClient):
    ...
    def __init__(self, *args: Any, **kwargs: Any):
        ...
        self.synthetic = SyntheticDatapointsAPI(self._config, api_version=self._api_version, cognite_client=self._cognite_client)

    def retrieve(self, *, id: Union[(None, int, Dict[(str, Any)], Sequence[Union[(int, Dict[(str, Any)])]])]=None, external_id: Union[(None, str, Dict[(str, Any)], Sequence[Union[(str, Dict[(str, Any)])]])]=None, start: Union[(int, str, datetime, None)]=None, end: Union[(int, str, datetime, None)]=None, aggregates: Union[(str, List[str], None)]=None, granularity: Optional[str]=None, limit: Optional[int]=None, include_outside_points: bool=False, ignore_unknown_ids: bool=False):
        query = _DatapointsQuery(start=start, end=end, id=id, external_id=external_id, aggregates=aggregates, granularity=granularity, limit=limit, include_outside_points=include_outside_points, ignore_unknown_ids=ignore_unknown_ids)
    ...
    def retrieve_dataframe(self, *, id: Union[(None, int, Dict[(str, Any)], Sequence[Union[(int, Dict[(str, Any)])]])]=None, external_id: Union[(None, str, Dict[(str, Any)], Sequence[Union[(str, Dict[(str, Any)])]])]=None, start: Union[(int, str, datetime, None)]=None, end: Union[(int, str, datetime, None)]=None, aggregates: Union[(str, List[str], None)]=None, granularity: Optional[str]=None, limit: Optional[int]=None, include_outside_points: bool=False, ignore_unknown_ids: bool=False, uniform_index: bool=False, include_aggregate_name: bool=True, include_granularity_name: bool=False, column_names: Literal[('id', 'external_id')]='external_id'):
        ...
        if uniform_index:
            ...
            if (fetcher.raw_queries or (len(grans_given) > 1) or is_limited):
                raise ValueError(f'Cannot return a uniform index when asking for aggregates with multiple granularities ({grans_given}) OR when (partly) querying raw datapoints OR when a finite limit is used.')
        df = fetcher.fetch_all_datapoints_numpy().to_pandas(column_names, include_aggregate_name, include_granularity_name)
    ...
    def insert(self, datapoints, id=None, external_id=None):
        ...
        dps_to_post: Union[(Sequence[Dict[(str, Union[(int, float, str, datetime)])]], Sequence[Tuple[(Union[(int, float, datetime)], Union[(int, float, str)])]])]
    ...
    def delete_ranges(self, ranges):
        ...
        for time_range in ranges:
            ...
            valid_range.update(inclusiveBegin=timestamp_to_ms(time_range['start']), exclusiveEnd=timestamp_to_ms(time_range['end']))
...
class DatapointsPoster():
    ...
    def _extract_raw_data_from_dps_container(dps):
        if (dps.value is None):
            raise ValueError('Only raw datapoints are supported when inserting data from `Datapoints` or `DatapointsArray`')
    ...
    def _validate_dps_objects(dps_object_list):
        ...
        for dps_object in dps_object_list:
            valid_dps_object = cast(Dict[(str, Union[(int, str, List[Tuple[(int, Any)]])])], validate_user_input_dict_with_identifier(dps_object, required_keys={'datapoints'}))
    ...
    def _insert_datapoints_concurrently(self, dps_object_lists):
        ...
        summary.raise_compound_exception_if_failed_tasks(task_unwrap_fn=(lambda x: x[0]), task_list_element_unwrap_fn=(lambda x: {k: x[k] for k in ['id', 'externalId'] if (k in x)}))
...
class RetrieveLatestDpsFetcher():
    ...
    def fetch_datapoints(self):
        tasks = [{'url_path': (self.dps_client._RESOURCE_PATH + '/latest'), 'json': {'items': chunk, 'ignoreUnknownIds': self.ignore_unknown_ids}} for chunk in split_into_chunks(self._all_identifiers, RETRIEVE_LATEST_LIMIT)]
