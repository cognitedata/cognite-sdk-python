...
class SyntheticDatapointsAPI(APIClient):
    ...
    def query(self, expressions, start, end, limit=None, variables=None, aggregate=None, granularity=None):
        ...
        tasks = []
        expressions_to_iterate = (expressions if (isinstance(expressions, Sequence) and (not isinstance(expressions, str))) else [expressions])
        for i in range(len(expressions_to_iterate)):
            (expression, short_expression) = self._build_expression(expressions_to_iterate[i], variables, aggregate, granularity)
            query = {'expression': expression, 'start': cognite.client.utils._time.timestamp_to_ms(start), 'end': cognite.client.utils._time.timestamp_to_ms(end)}
            values: List[float] = []
        ...
        datapoints_summary = utils._concurrency.execute_tasks(self._fetch_datapoints, tasks, max_workers=self._config.max_workers)
        if datapoints_summary.exceptions:
            raise datapoints_summary.exceptions[0]
        return (DatapointsList(datapoints_summary.results, cognite_client=self._cognite_client) if isinstance(expressions, List) else datapoints_summary.results[0])
    ...
    def _build_expression(expression, variables=None, aggregate=None, granularity=None):
        if expression.__class__.__module__.startswith('sympy.'):
            ...
            if (not variables):
                raise ValueError('sympy expressions are only supported in combination with the `variables` parameter to map symbols to time series.')
        else:
        ...
        if variables:
            for (k, v) in variables.items():
                if isinstance(v, TimeSeries):
                    v = v.external_id
                expression_with_ts = re.sub(re.compile(f'\b{k}\b'), f"ts{{externalId:'{v}'{aggregate_str}}}", expression_with_ts)
    ...
    def _sympy_to_sts(expression):
        ...
        functions = {sympy_module.cos: 'cos', sympy_module.sin: 'sin', sympy_module.sqrt: 'sqrt', sympy_module.log: 'ln', sympy_module.exp: 'exp', sympy_module.Abs: 'abs'}
