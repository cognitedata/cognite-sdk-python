...
from cognite.client.data_classes.transformations import NonceCredentials, TagsFilter, TransformationFilter, TransformationPreviewResult, TransformationUpdate
...
class TransformationsAPI(APIClient):
    ...
    def list(self, include_public=True, name_regex=None, query_regex=None, destination_type=None, conflict_mode=None, cdf_project_name=None, has_blocked_error=None, created_time=None, last_updated_time=None, data_set_ids=None, data_set_external_ids=None, tags=None, limit=25):
        ...
        filter = TransformationFilter(include_public=include_public, name_regex=name_regex, query_regex=query_regex, destination_type=destination_type, conflict_mode=conflict_mode, cdf_project_name=cdf_project_name, has_blocked_error=has_blocked_error, created_time=created_time, last_updated_time=last_updated_time, tags=tags, data_set_ids=ds_ids).dump(camel_case=True)
        return self._list(list_cls=TransformationList, resource_cls=Transformation, method='POST', url_path=f'{self._RESOURCE_PATH}/filter', limit=limit, filter=filter)

    def retrieve(self, id=None, external_id=None):
        identifiers = IdentifierSequence.load(ids=id, external_ids=external_id).as_singleton()
        return self._retrieve_multiple(list_cls=TransformationList, resource_cls=Transformation, identifiers=identifiers)

    def retrieve_multiple(self, ids=None, external_ids=None, ignore_unknown_ids=False):
        identifiers = IdentifierSequence.load(ids=ids, external_ids=external_ids)
        return self._retrieve_multiple(list_cls=TransformationList, resource_cls=Transformation, identifiers=identifiers, ignore_unknown_ids=ignore_unknown_ids)
    ...
    def update(self, item):
        if isinstance(item, Sequence):
            ...
            raise TypeError('item must be Sequence[Transformation], Transformation, Sequence[TransformationUpdate] or TransformationUpdate')
        return self._update_multiple(list_cls=TransformationList, resource_cls=Transformation, update_cls=TransformationUpdate, items=item)
    ...
    def run_async(self, transformation_id=None, transformation_external_id=None, timeout=None):
        job = self.run(transformation_id=transformation_id, transformation_external_id=transformation_external_id, wait=False)
    ...
    def preview(self, query=None, convert_to_string=False, limit=100, source_limit=100, infer_schema_limit=1000):
        request_body = {'query': query, 'convertToString': convert_to_string, 'limit': limit, 'sourceLimit': source_limit, 'inferSchemaLimit': infer_schema_limit}
