...
class GeoLocation(dict):
    ...
    def _load(cls, raw_geo_location=None, **kwargs: Dict[(str, Any)]):
        raw_geo_location = cast(Dict[(str, Any)], handle_deprecated_camel_case_argument(raw_geo_location, 'raw_geoLocation', '_load', kwargs))
        return cls(type=raw_geo_location.get('type', 'Feature'), geometry=raw_geo_location['geometry'], properties=raw_geo_location.get('properties'))
...
class GeoLocationFilter(dict):
    ...
    def _load(cls, raw_geo_location_filter=None, **kwargs: Dict[(str, Any)]):
        raw_geo_location_filter = cast(Dict[(str, Any)], handle_deprecated_camel_case_argument(raw_geo_location_filter, 'raw_geoLocation_filter', '_load', kwargs))
