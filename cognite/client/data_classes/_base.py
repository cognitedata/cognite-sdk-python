...
class CognitePropertyClassUtil():
    ...
    def declare_property(schema_name):
        return property((lambda s: (s[schema_name] if (schema_name in s) else None))).setter((lambda s, v: CognitePropertyClassUtil._property_setter(s, schema_name, v))).deleter((lambda s: s.pop(schema_name, None)))
    ...
    def __init__(self, resources, cognite_client=None):
        for resource in resources:
            if (not isinstance(resource, self._RESOURCE)):
                raise TypeError(f"All resources for class '{self.__class__.__name__}' must be of type '{self._RESOURCE.__name__}', not '{type(resource)}'.")
...
class CogniteUpdate():
    ...
    def _set(self, name, value):
        ...
        assert (('remove' not in update_obj) and ('add' not in update_obj)), 'Can not call set after adding or removing fields from an update object.'
    ...
    def _modify(self, name, value):
        update_obj = self._update_object.get(name, {})
        assert ('set' not in update_obj), 'Can not call remove or add fields after calling set on an update object.'
        assert ('modify' not in update_obj), 'Can not call modify twice on the same object, please combine your items and pass them to modify in one call.'
