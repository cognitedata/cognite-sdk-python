...
class EdgeConnection(ConnectionDefinition, ABC):
    ...
    Args:
        type: Reference to the node pointed to by the direct relation. The reference consists of a space and an external-id.
        ...
        edge_source: The edge(s) of this connection can be read through the view specified in 'edgeSource'.
        direction: The direction of the edge. The outward direction is used to indicate that the edge points from the source to the target. The inward direction is used to indicate that the edge points from the target to the source.
    """
