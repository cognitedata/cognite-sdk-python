...
class FeatureType(CogniteResource):
    ...
    def __init__(self, external_id=None, data_set_id=None, created_time=None, last_updated_time=None, properties=None, search_spec=None, cognite_client=None):
...
class Feature(CogniteResource):
    ...
    def dump(self, camel_case=False):
        ...
        def to_camel_case(key: str) -> str:
            ...
            return key
        return {to_camel_case(key): value for (key, value) in self.__dict__.items() if ((value is not None) and (not key.startswith('_')))}

def _is_geometry_type(property_type):
    return (property_type in {'GEOMETRY', 'POINT', 'LINESTRING', 'POLYGON', 'MULTIPOINT', 'MULTILINESTRING', 'MULTIPOLYGON', 'GEOMETRYCOLLECTION', 'GEOMETRYZ', 'POINTZ', 'LINESTRINGZ', 'POLYGONZ', 'MULTIPOINTZ', 'MULTILINESTRINGZ', 'MULTIPOLYGONZ', 'GEOMETRYCOLLECTIONZ', 'GEOMETRYM', 'POINTM', 'LINESTRINGM', 'POLYGONM', 'MULTIPOINTM', 'MULTILINESTRINGM', 'MULTIPOLYGONM', 'GEOMETRYCOLLECTIONM', 'GEOMETRYZM', 'POINTZM', 'LINESTRINGZM', 'POLYGONZM', 'MULTIPOINTZM', 'MULTILINESTRINGZM', 'MULTIPOLYGONZM', 'GEOMETRYCOLLECTIONZM'})
...
class FeatureList(CogniteResourceList):
    ...
    @staticmethod
    def from_geopandas(feature_type, geodataframe, external_id_column='externalId', property_column_mapping=None, data_set_id_column='dataSetId'):
        features = []
        ...
        for (_, row) in geodataframe.iterrows():
            ...
            for prop in feature_type.properties.items():
                prop_name = prop[0]
                if (prop_name.startswith('_') or (prop_name in ['createdTime', 'lastUpdatedTime', 'externalId', 'dataSetId'])):
                    continue
...
class GeospatialGeometryTransformComputeFunction(GeospatialComputeFunction):
    ...
    def to_json_payload(self):
        return {'stTransform': {'geometry': self.geospatial_geometry_compute_function.to_json_payload(), 'srid': self.srid}}
...
class GeospatialComputedResponse(CogniteResource):
    ...
    def _load(cls, resource, cognite_client=None):
        if isinstance(resource, str):
            return cls._load(json.loads(resource), cognite_client=cognite_client)
        item_list = GeospatialComputedItemList._load(cast('List[Any]', resource.get('items')), cognite_client=cognite_client)
