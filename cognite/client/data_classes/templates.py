...
from cognite.client.data_classes._base import EXCLUDE_VALUE, CogniteObjectUpdate, CogniteResource, CogniteResourceList, CogniteUpdate
if TYPE_CHECKING:
    from cognite.client import CogniteClient

class TemplateGroup(CogniteResource):

    def __init__(self, external_id=None, description=None, owners=None, data_set_id=None, created_time=None, last_updated_time=None, cognite_client=None):
...
class TemplateGroupVersion(CogniteResource):
    ...
    def __init__(self, schema=None, version=None, conflict_mode=None, created_time=None, last_updated_time=None, cognite_client=None):
        self.schema = schema
...
class SyntheticTimeSeriesResolver(CogniteResource):
    ...
    def __init__(self, expression=None, name=None, description=None, metadata=None, is_step=None, is_string=None, unit=None, cognite_client=None):
...
class TemplateInstance(CogniteResource):
    ...
    def __init__(self, external_id=None, template_name=None, field_resolvers=None, data_set_id=None, created_time=None, last_updated_time=None, cognite_client=None):
        self.external_id = external_id
        ...
        self.last_updated_time = last_updated_time
        self._cognite_client = cast('CogniteClient', cognite_client)
    field_resolver_mapper: Dict[(str, Type[CogniteResource])] = {'constant': ConstantResolver, 'syntheticTimeSeries': SyntheticTimeSeriesResolver, 'raw': RawResolver, 'view': ViewResolver}

    def dump(self, camel_case=False):
        if camel_case:
            return {utils._auxiliary.to_camel_case(key): (value if (key != 'field_resolvers') else TemplateInstance._encode_field_resolvers(value, camel_case=camel_case)) for (key, value) in self.__dict__.items() if ((value not in EXCLUDE_VALUE) and (not key.startswith('_')))}
        return {key: (value if (key != 'field_resolvers') else TemplateInstance._encode_field_resolvers(value, camel_case=camel_case)) for (key, value) in self.__dict__.items() if ((value not in EXCLUDE_VALUE) and (not key.startswith('_')))}

    @staticmethod
    def _encode_field_resolvers(field_resolvers, camel_case):
        return {key: (value.dump(camel_case=camel_case) if (not isinstance(value, str)) else value) for (key, value) in field_resolvers.items()}
            ...
            for (key, value) in resource.items():
                ...
                if hasattr(instance, snake_case_key):
                    if (key == 'fieldResolvers'):
                        setattr(instance, snake_case_key, {key: TemplateInstance._field_resolver_load(field_resolver) for (key, field_resolver) in value.items()})
                    else:
...
class View(CogniteResource):
    ...
    def __init__(self, external_id=None, source=None, data_set_id=None, created_time=None, last_updated_time=None, cognite_client=None):
        self.external_id = external_id
        self.source = source
        self.data_set_id = data_set_id
        self.created_time = created_time
    ...
    def dump(self, camel_case=False):
        if camel_case:
            return {utils._auxiliary.to_camel_case(key): View.resolve_nested_classes(value, camel_case) for (key, value) in self.__dict__.items() if ((value not in EXCLUDE_VALUE) and (not key.startswith('_')))}
        return {key: View.resolve_nested_classes(value, camel_case) for (key, value) in self.__dict__.items() if ((value not in EXCLUDE_VALUE) and (not key.startswith('_')))}
