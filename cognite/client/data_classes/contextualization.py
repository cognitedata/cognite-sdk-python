...
from cognite.client.data_classes._base import CognitePrimitiveUpdate, CogniteResource, CogniteResourceList, CogniteUpdate
...
class JobStatus(Enum):
    ...
    def is_not_finished(self):
        return (self in {JobStatus._NOT_STARTED, JobStatus.QUEUED, JobStatus.RUNNING, JobStatus.DISTRIBUTED, JobStatus.DISTRIBUTING, JobStatus.COLLECTING})
...
class ContextualizationJob(CogniteResource):
    _COMMON_FIELDS = frozenset({'status', 'jobId', 'modelId', 'pipelineId', 'errorMessage', 'createdTime', 'startTime', 'statusTime'})
    _JOB_TYPE = ContextualizationJobType.ENTITY_MATCHING

    def __init__(self, job_id=None, model_id=None, status=None, error_message=None, created_time=None, start_time=None, status_time=None, status_path=None, cognite_client=None):
        self.job_id = job_id
...
class EntityMatchingModel(CogniteResource):
    ...
    def __init__(self, id=None, status=None, error_message=None, created_time=None, start_time=None, status_time=None, classifier=None, feature_type=None, match_fields=None, model_type=None, name=None, description=None, external_id=None, cognite_client=None):
        self.id = id
        self.status = status
        self.created_time = created_time
        self.start_time = start_time
    ...
    def predict(self, sources=None, targets=None, num_matches=1, score_threshold=None):
        self.wait_for_completion()
        return self._cognite_client.entity_matching._run_job(job_path='/predict', job_cls=ContextualizationJob, status_path='/jobs/', json={'id': self.id, 'sources': self._dump_entities(sources), 'targets': self._dump_entities(targets), 'numMatches': num_matches, 'scoreThreshold': score_threshold})

    def refit(self, true_matches):
        true_matches = [convert_true_match(true_match) for true_match in true_matches]
        self.wait_for_completion()
        response = self._cognite_client.entity_matching._post((self._RESOURCE_PATH + '/refit'), json={'trueMatches': true_matches, 'id': self.id})
...
class DiagramConvertResults(ContextualizationJob):
    ...
    def __getitem__(self, find_id):
        found = [item for item in self.result['items'] if ((item.get('fileId') == find_id) or (item.get('fileExternalId') == find_id))]
        if (not found):
    ...
    def items(self):
        if (JobStatus(self.status) is JobStatus.COMPLETED):
            self._items = [DiagramConvertItem._load(item, cognite_client=self._cognite_client) for item in self.result['items']]
        return self._items
...
class DiagramDetectItem(CogniteResource):
    ...
    def __init__(self, file_id=None, file_external_id=None, annotations=None, error_message=None, cognite_client=None, page_range=None):
        self.file_id = file_id
        self.file_external_id = file_external_id
...
class DiagramDetectResults(ContextualizationJob):
    ...
    def items(self):
        if (JobStatus(self.status) is JobStatus.COMPLETED):
            self._items = [DiagramDetectItem._load(item, cognite_client=self._cognite_client) for item in self.result['items']]
        return self._items
...
VISION_FEATURE_MAP: Dict[(str, Any)] = {'text_predictions': TextRegion, 'asset_tag_predictions': AssetLink, 'industrial_object_predictions': ObjectDetection, 'people_predictions': ObjectDetection, 'personal_protective_equipment_predictions': ObjectDetection}
VISION_ANNOTATION_TYPE_MAP: Dict[(str, str)] = {'text_predictions': 'images.TextRegion', 'asset_tag_predictions': 'images.AssetLink', 'industrial_object_predictions': 'images.ObjectDetection', 'people_predictions': 'images.ObjectDetection', 'personal_protective_equipment_predictions': 'images.ObjectDetection'}
...
class DetectJobBundle():
    ...
    def __init__(self, job_ids, cognite_client=None):
        warnings.warn('DetectJobBundle.result is calling a beta endpoint which is still in development. Breaking changes can happen in between patch versions.')
        self._cognite_client = cast('CogniteClient', cognite_client)
        if (not job_ids):
...
class VisionExtractItem(CogniteResource):
    ...
    def dump(self, camel_case=False):
        ...
        if (('predictions' in item_dump) and isinstance(self._predictions_dict, Dict)):
            item_dump['predictions'] = (self._predictions_dict if camel_case else self._resource_to_snake_case(self._predictions_dict))
        return item_dump
...
class VisionExtractJob(VisionJob):
    ...
    def items(self):
        if (JobStatus(self.status) is JobStatus.COMPLETED):
            self._items = [VisionExtractItem._load(item, cognite_client=self._cognite_client) for item in self.result['items']]
        return self._items
    ...
    def _predictions_to_annotations(self, creating_user=None, creating_app=None, creating_app_version=None):
        return [Annotation(annotated_resource_id=item.file_id, annotation_type=VISION_ANNOTATION_TYPE_MAP[prediction_type], data=data.dump(), annotated_resource_type='file', status='suggested', creating_app=(creating_app or 'cognite-sdk-python'), creating_app_version=(creating_app_version or self._cognite_client.version), creating_user=(creating_user or None)) for item in (self.items or []) if (item.predictions is not None) for (prediction_type, prediction_data_list) in item.predictions.dump().items() for data in prediction_data_list]

    def save_predictions(self, creating_user=None, creating_app=None, creating_app_version=None):
        if (JobStatus(self.status) is JobStatus.COMPLETED):
            annotations = self._predictions_to_annotations(creating_user=creating_user, creating_app=creating_app, creating_app_version=creating_app_version)
            if annotations:
        ...
        raise CogniteException('Extract job is not completed. If the job is queued or running, wait for completion and try again')
