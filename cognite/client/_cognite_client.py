...
class CogniteClient:
    ...
    def __init__(self, config: Optional[ClientConfig] = None) -> None:
        ...
        self.assets = AssetsAPI(self._config, cognite_client=self)
        self.events = EventsAPI(self._config, cognite_client=self)
        self.files = FilesAPI(self._config, cognite_client=self)
        self.iam = IAMAPI(self._config, cognite_client=self)
        self.data_sets = DataSetsAPI(self._config, cognite_client=self)
        self.sequences = SequencesAPI(self._config, cognite_client=self)
        self.time_series = TimeSeriesAPI(self._config, cognite_client=self)
        self.geospatial = GeospatialAPI(self._config, cognite_client=self)
        self.raw = RawAPI(self._config, cognite_client=self)
        self.three_d = ThreeDAPI(self._config, cognite_client=self)
        self.labels = LabelsAPI(self._config, cognite_client=self)
        self.relationships = RelationshipsAPI(self._config, cognite_client=self)
        self.entity_matching = EntityMatchingAPI(self._config, cognite_client=self)
        self.templates = TemplatesAPI(self._config, cognite_client=self)
        self.vision = VisionAPI(self._config, cognite_client=self)
        self.extraction_pipelines = ExtractionPipelinesAPI(self._config, cognite_client=self)
        self.transformations = TransformationsAPI(self._config, cognite_client=self)
        self.diagrams = DiagramsAPI(self._config, cognite_client=self)
        self.annotations = AnnotationsAPI(self._config, cognite_client=self)
        self.functions = FunctionsAPI(self._config, cognite_client=self)
