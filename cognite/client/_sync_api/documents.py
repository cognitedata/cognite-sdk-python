"""
===============================================================================
9381129680f630eb55f697b03a7260fd
...
class SyncDocumentsAPI:
    ...
    def __call__(
        ...
        Returns:
            Iterator[Document | DocumentList]: yields Documents one by one if chunk_size is not specified, else DocumentList objects.
        """
        return SyncIterator(
            self.__async_client.documents(
                chunk_size=chunk_size, filter=filter, sort=sort, limit=limit, partitions=partitions
            )
        )
    ...
    def aggregate_count(self, query: str | None = None, filter: Filter | dict[str, Any] | None = None) -> int:
        ...
        Examples:
            ...
            the specified external IDs, e.g. 'Plant_1' and 'Plant_2':
                ...
                ... )
        """
        return self._run_sync(self.__async_client.documents.aggregate_count(query=query, filter=filter))
    ...
    def retrieve_content(self, id: int) -> bytes:
        ...
        """
        return self._run_sync(self.__async_client.documents.retrieve_content(id=id))
    ...
    def retrieve_content_buffer(self, id: int, buffer: BinaryIO) -> None:
        ...
        """
        return self._run_sync(self.__async_client.documents.retrieve_content_buffer(id=id, buffer=buffer))
    ...
    def list(
        ...
        """
        return self._run_sync(self.__async_client.documents.list(filter=filter, sort=sort, limit=limit))
