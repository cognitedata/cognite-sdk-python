"""
===============================================================================
c96828711ef1e904deb4f5cfbca68cff
...
class SyncUsersAPI:
    ...
    def __call__(self, chunk_size: int | None = None, limit: int | None = None) -> Iterator[User | UserList]:
        ...
        Returns:
            Iterator[User | UserList]: No description.
        Yields:
            User | UserList: yields User one by one if chunk_size is not specified, else UserList objects.
        """
        return SyncIterator(self.__async_client.postgres_gateway.users(chunk_size=chunk_size, limit=limit))
    ...
    def create(self, user: UserWrite | Sequence[UserWrite]) -> UserCreated | UserCreatedList:
        ...
        """
        return self._run_sync(self.__async_client.postgres_gateway.users.create(user=user))
    ...
    def update(self, items: UserUpdate | UserWrite | Sequence[UserUpdate | UserWrite]) -> User | UserList:
        ...
        """
        return self._run_sync(self.__async_client.postgres_gateway.users.update(items=items))
    ...
    def list(self, limit: int = DEFAULT_LIMIT_READ) -> UserList:
        ...
        """
        return self._run_sync(self.__async_client.postgres_gateway.users.list(limit=limit))
