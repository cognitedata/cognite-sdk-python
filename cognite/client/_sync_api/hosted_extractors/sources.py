"""
===============================================================================
c2d006cdc98692d7f40ea5cdd5af4a7b
...
class SyncSourcesAPI:
    ...
    def __call__(self, chunk_size: int | None = None, limit: int | None = None) -> Iterator[Source | SourceList]:
        ...
        Returns:
            Iterator[Source | SourceList]: No description.
        Yields:
            Source | SourceList: yields Source one by one if chunk_size is not specified, else SourceList objects.
        """
        return SyncIterator(self.__async_client.hosted_extractors.sources(chunk_size=chunk_size, limit=limit))
    ...
    def create(self, items: SourceWrite | Sequence[SourceWrite]) -> Source | SourceList:
        ...
        """
        return self._run_sync(self.__async_client.hosted_extractors.sources.create(items=items))
    ...
    def update(
        ...
        """
        return self._run_sync(self.__async_client.hosted_extractors.sources.update(items=items, mode=mode))
    ...
    def list(self, limit: int | None = DEFAULT_LIMIT_READ) -> SourceList:
        ...
        """
        return self._run_sync(self.__async_client.hosted_extractors.sources.list(limit=limit))
