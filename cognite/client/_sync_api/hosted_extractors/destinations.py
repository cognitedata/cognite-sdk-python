"""
===============================================================================
361612950aed4862fb660116176ccce0
...
class SyncDestinationsAPI:
    ...
    @overload
    def __call__(self, chunk_size: None = None, limit: int | None = None) -> AsyncIterator[Destination]: ...

    @overload
    def __call__(self, chunk_size: int, limit: int | None = None) -> AsyncIterator[Destination]: ...
    ...
    def __call__(
        ...
        Returns:
            Iterator[Destination | DestinationList]: No description.
        Yields:
            Destination | DestinationList: yields Destination one by one if chunk_size is not specified, else DestinationList objects.
        """
        return SyncIterator(self.__async_client.hosted_extractors.destinations(chunk_size=chunk_size, limit=limit))
    ...
    def create(self, items: DestinationWrite | Sequence[DestinationWrite]) -> Destination | DestinationList:
        ...
        """
        return self._run_sync(self.__async_client.hosted_extractors.destinations.create(items=items))
    ...
    def update(
        ...
        """
        return self._run_sync(self.__async_client.hosted_extractors.destinations.update(items=items, mode=mode))
    ...
    def list(self, limit: int | None = DEFAULT_LIMIT_READ) -> DestinationList:
        ...
        """
        return self._run_sync(self.__async_client.hosted_extractors.destinations.list(limit=limit))
