"""
===============================================================================
9c5ef7d6569918838ccdd7e707c0404c
...
class SyncJobsAPI:
    ...
    @overload
    def __call__(self, chunk_size: None = None, limit: int | None = None) -> AsyncIterator[Job]: ...
    ...
    def __call__(self, chunk_size: int | None = None, limit: int | None = None) -> Iterator[Job | JobList]:
        ...
        Returns:
            Iterator[Job | JobList]: No description.
        Yields:
            Job | JobList: yields Job one by one if chunk_size is not specified, else JobList objects.
        """
        return SyncIterator(self.__async_client.hosted_extractors.jobs(chunk_size=chunk_size, limit=limit))
    ...
    def create(self, items: JobWrite | Sequence[JobWrite]) -> Job | JobList:
        ...
        """
        return self._run_sync(self.__async_client.hosted_extractors.jobs.create(items=items))
    ...
    def update(
        ...
        """
        return self._run_sync(self.__async_client.hosted_extractors.jobs.update(items=items, mode=mode))
    ...
    def list(self, limit: int | None = DEFAULT_LIMIT_READ) -> JobList:
        ...
        """
        return self._run_sync(self.__async_client.hosted_extractors.jobs.list(limit=limit))
