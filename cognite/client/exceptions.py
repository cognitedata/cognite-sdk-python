...
class CogniteMultiException(CogniteException):
    ...
    def _get_multi_exception_summary(self) -> str:
        ...
        summary = [
            ...
            f"- Successful (2xx): {len(self.successful)} requests",
            f"- Unknown (5xx): {len(self.unknown)} requests",
            f"- Failed (4xx): {len(self.failed)} requests",
        ]
        # Only show 'skipped' when tasks were skipped to avoid confusion:
        if skipped := self.skipped:
            summary.append(f"- Skipped: {len(skipped)} requests")
        return "\n".join(summary)
