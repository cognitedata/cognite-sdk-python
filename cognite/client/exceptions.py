...
class CogniteProjectAccessError(CogniteException):
    ...
    async def _attempt_to_get_projects(client: AsyncCogniteClient, current_project: str) -> list[str] | None:
        ...
        try:
            token_inspect = await api_client._http_client.request(
                "GET", full_url, headers=full_headers, timeout=5, semaphore=None
            )
...
class CogniteHTTPStatusError(CogniteRequestError):
    ...
    def __init__(self, status_code: int, *, request: HttpxRequest, response: CogniteHTTPResponse) -> None:
...
class CogniteMissingClientError(CogniteException):
    ...
    def __str__(self) -> str:
        return (
            ...
            "You can set the client by calling '.set_client_ref(my_client)'"
        )
