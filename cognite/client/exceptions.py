...
class CogniteMultiException(CogniteException):
    ...
    def _get_multi_exception_summary(self):
        if ((len(self.successful) == 0) and (len(self.unknown) == 0) and (len(self.failed) == 0)):
            return ''
        return '\n'.join(('', 'The API Failed to process some items.', f'Successful (2xx): {list(map(self._unwrap_fn, self.successful))}', f'Unknown (5xx): {list(map(self._unwrap_fn, self.unknown))}', f'Failed (4xx): {list(map(self._unwrap_fn, self.failed))}'))

class CogniteAPIError(CogniteMultiException):

    def __init__(self, message, code, x_request_id=None, missing=None, duplicated=None, successful=None, failed=None, unknown=None, unwrap_fn=None, extra=None):
