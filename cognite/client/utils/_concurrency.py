...
class AsyncSDKTask:
    ...
    def as_async_task(self) -> asyncio.Task:
    ...
    def async_task(self) -> asyncio.Task:
        if self._async_task is None:
            raise RuntimeError("Task has not been created yet")
...
async def execute_async_tasks_with_fail_fast(tasks: list[AsyncSDKTask]) -> TasksSummary:
    # If no future raises an exception then this is equivalent to asyncio.ALL_COMPLETED:
        ...
        [task.as_async_task() for task in tasks],
    ...
    # Something failed, and because of fail-fast, we cancel all pending tasks:
    for task in pending:
        task.cancel()
