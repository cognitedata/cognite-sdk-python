...
def handle_deprecated_camel_case_argument(new_arg, old_arg_name, fn_name, kw_dct):
    ...
    if (old_arg is None):
        ...
        return new_arg
    warnings.warn(f"Argument '{old_arg_name}' have been changed to '{new_arg_name}', but the old is still supported until the next major version. Consider updating your code.", UserWarning, stacklevel=2)
...
def validate_user_input_dict_with_identifier(dct, required_keys):
    ...
    if (missing_keys or invalid_keys):
        raise ValueError(f'Given dictionary failed validation. Invalid key(s): {sorted(invalid_keys)}, required key(s) missing: {sorted(missing_keys)}.')
    return id_dct
...
def _check_client_has_newest_major_version():
    ...
    if (newest_version != version):
        warnings.warn(f'''You are using version={version} of the SDK, however version='{newest_version}' is available. To suppress this warning, either upgrade or do the following:
>>> from cognite.client.config import global_config
>>> global_config.disable_pypi_version_check = True''', stacklevel=3)
