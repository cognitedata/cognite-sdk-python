...
def assert_exactly_one_of_id_or_external_id(id: int, external_id: str) -> Dict[str, Union[str, int]]:
...
def assert_at_least_one_of_id_or_external_id(id: int, external_id: str) -> Dict[str, Union[str, int]]:
    assert_type(id, "id", [numbers.Integral], allow_none=True)
    assert_type(external_id, "external_id", [str], allow_none=True)
    has_id = id is not None
    has_external_id = external_id is not None
...
def convert_true_match(true_match: Union[dict, list]) -> dict:
