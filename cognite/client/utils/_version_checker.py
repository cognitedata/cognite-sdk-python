...
def check_if_version_exists(package_name, version):
    versions = get_all_versions(package_name=package_name)
    return version in versions


@functools.lru_cache(maxsize=1)
def get_newest_version_in_major_release(package_name, version):
...
def get_all_versions(package_name):
    ...
    return re.findall("cognite-sdk-(\\d+\\.\\d+.[\\dabrc]+)", res.content.decode())
