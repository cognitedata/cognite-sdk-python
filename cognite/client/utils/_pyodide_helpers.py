...
def patch_sdk_for_pyodide():
    ...
    warnings.filterwarnings(action='ignore', category=UserWarning, message="Your installation of 'protobuf' is missing compiled C binaries")
...
class FusionNotebookConfig(ClientConfig):
    ...
    def __init__(self, client_name='DSHubLite', api_subversion=None, headers=None, timeout=None, file_transfer_timeout=None, debug=False):
        super().__init__(client_name=client_name, api_subversion=api_subversion, headers=headers, timeout=timeout, file_transfer_timeout=file_transfer_timeout, debug=debug, project=os.environ['COGNITE_PROJECT'], credentials=EnvVarToken(), base_url=os.environ['COGNITE_BASE_URL'], max_workers=1)
