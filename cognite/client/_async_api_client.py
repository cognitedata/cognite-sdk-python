...
class AsyncAPIClient:
    ...
    async def _list(
        ...
        items = []
        async for chunk in self._list_generator(method, list_cls, resource_cls, **fetch_kwargs):
        ...
        async def get_partition(partition: int) -> list[dict[str, Any]]:
            ...
            while True:
                if method == "POST":
                    ...
                    res = await self._get(url_path=(resource_path or self._RESOURCE_PATH), params=params, headers=headers)
                else:
    ...
    async def _delete_multiple(
        ...
        summary = await execute_tasks_async(self._post, tasks, max_workers=self._config.max_workers, executor=executor)
        summary.raise_compound_exception_if_failed_tasks(
            task_unwrap_fn=unpack_items_in_payload,
