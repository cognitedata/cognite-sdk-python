...
class _OAuthCredentialProviderWithTokenRefresh(CredentialProvider):
    ...
    def _verify_credentials(credentials):
        ...
        raise CogniteAuthError(f"Error generating access token! Error: {credentials['error']}, error description: {err_descr}")
...
class OAuthDeviceCode(_OAuthCredentialProviderWithTokenRefresh, _WithMsalSerializableTokenCache):
    ...
    def __init__(self, authority_url, client_id, scopes, token_cache_path=None):
        ...
        self.__app = PublicClientApplication(client_id=self.__client_id, authority=self.__authority_url, token_cache=serializable_token_cache, verify=(not global_config.disable_ssl))
...
class OAuthClientCredentials(_OAuthCredentialProviderWithTokenRefresh):
    ...
    def _validate_token_custom_args(self):
        ...
        if bad_args:
            raise TypeError(f'The following reserved token custom arg(s) were passed: {sorted(bad_args)}. The full list of reserved custom args is: {sorted(reserved)}.')
    ...
    def _refresh_access_token(self):
        from cognite.client.config import global_config
        try:
            token_result = self.__oauth.fetch_token(token_url=self.__token_url, verify=(not global_config.disable_ssl), client_secret=self.__client_secret, **self.__token_custom_args)
            return (token_result['access_token'], token_result['expires_at'])
        except OAuth2Error as oauth_err:
            raise CogniteAuthError(f'Error generating access token: {oauth_err.error}, {oauth_err.status_code}, {oauth_err.description}') from oauth_err
