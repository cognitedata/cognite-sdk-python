...
from cognite.client.data_classes._base import CogniteFilter, CogniteResource, CogniteUpdate, T_CogniteResource
...
class APIClient:
    ...
    def _url_is_retryable(cls, url):
        valid_url_pattern = (
            "^(?:http|https)://[a-z\\d.:\\-]+(?:/api/(?:v1|playground)/projects/[^/]+)?((/[^\\?]+)?(\\?.+)?)"
        )
    ...
    def _list_generator_partitioned(
        self, partitions, resource_cls, resource_path, filter=None, other_params=None, headers=None
    ):
    ...
    def _list_partitioned(
        self, partitions, method, list_cls, resource_path=None, filter=None, other_params=None, headers=None
    ):
    ...
    def _delete_multiple(
        self, identifiers, wrap_ids, resource_path=None, params=None, headers=None, extra_body_fields=None
    ):
        resource_path = resource_path or self._RESOURCE_PATH
        tasks = [
            {
    ...
    def _update_multiple(
        self, items, list_cls, resource_cls, update_cls, resource_path=None, params=None, headers=None
    ):
        ...
