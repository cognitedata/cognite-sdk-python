...
from cognite.client.data_classes._base import CogniteFilter, CogniteResource, CogniteUpdate, T_CogniteResource, T_CogniteResourceList
...
class APIClient():
    _RESOURCE_PATH: str
    _RETRYABLE_POST_ENDPOINT_REGEX_PATTERNS = {f'^{path}(\?.*)?$' for path in ('/(assets|events|files|timeseries|sequences|datasets|relationships)/(list|byids|search|aggregate)', '/files/downloadlink', '/timeseries/data', '/timeseries/data/(list|latest|delete)', '/sequences/data', '/sequences/data/(list|delete)', '/raw/dbs/[^/]+/tables/[^/]+', '/context/entitymatching/(byids|list|jobs)', '/sessions/revoke')}
    ...
    def __init__(self, config, api_version=None, cognite_client=None):
        ...
        self._http_client = HTTPClient(config=HTTPClientConfig(status_codes_to_retry={429}, backoff_factor=0.5, max_backoff_seconds=global_config.max_retry_backoff, max_retries_total=global_config.max_retries, max_retries_read=0, max_retries_connect=global_config.max_retries, max_retries_status=global_config.max_retries), session=session)
        self._http_client_with_retry = HTTPClient(config=HTTPClientConfig(status_codes_to_retry=global_config.status_forcelist, backoff_factor=0.5, max_backoff_seconds=global_config.max_retry_backoff, max_retries_total=global_config.max_retries, max_retries_read=global_config.max_retries, max_retries_connect=global_config.max_retries, max_retries_status=global_config.max_retries), session=session)
    ...
    def _delete(self, url_path, params=None, headers=None):
    ...
    def _post(self, url_path, json=None, params=None, headers=None):
        return self._do_request('POST', url_path, json=json, headers=headers, params=params, timeout=self._config.timeout)
    ...
    def _do_request(self, method, url_path, accept='application/json', **kwargs: Any):
        ...
        if json_payload:
            try:
                ...
                if (msg in str(e)):
                    raise ValueError(f'{msg}. Make sure your data does not contain NaN(s) or +/- Inf!').with_traceback(e.__traceback__) from None
                raise
            kwargs['data'] = data
    ...
    def _configure_headers(self, accept, additional_headers):
    ...
    def _resolve_url(self, method, url_path):
    ...
    def _url_is_retryable(cls, url):
        valid_url_pattern = '^(?:http|https)://[a-z\\d.:\\-]+(?:/api/(?:v1|playground)/projects/[^/]+)?((/[^\\?]+)?(\\?.+)?)'
    ...
    def _retrieve(self, identifier, cls, resource_path=None, params=None, headers=None):
        resource_path = (resource_path or self._RESOURCE_PATH)
        try:
            res = self._get(url_path=utils._auxiliary.interpolate_and_url_encode((resource_path + '/{}'), str(identifier.as_primitive())), params=params, headers=headers)
    ...
    @overload
    def _retrieve_multiple(self, list_cls, resource_cls, identifiers, resource_path=None, ignore_unknown_ids=None, headers=None, other_params=None):
        ...
    ...
    def _retrieve_multiple(self, list_cls, resource_cls, identifiers, resource_path=None, ignore_unknown_ids=None, headers=None, other_params=None):
        ...
        tasks: List[Dict[(str, Union[(str, Dict[(str, Any)], None)])]] = [{'url_path': (resource_path + '/byids'), 'json': {'items': id_chunk.as_dicts(), **ignore_unknown_obj, **(other_params or {})}, 'headers': headers} for id_chunk in identifiers.chunked(self._RETRIEVE_LIMIT)]
    ...
    def _list_generator(self, method, list_cls, resource_cls, resource_path=None, url_path=None, limit=None, chunk_size=None, filter=None, sort=None, other_params=None, partitions=None, headers=None, initial_cursor=None):
        ...
        if partitions:
            ...
            (yield from self._list_generator_partitioned(partitions=partitions, resource_cls=resource_cls, resource_path=resource_path, filter=filter, other_params=other_params, headers=headers))
        else:
    ...
    def _list_generator_partitioned(self, partitions, resource_cls, resource_path, filter=None, other_params=None, headers=None):
        ...
        def get_partition(partition_num: int) -> List[Dict[(str, Any)]]:
            ...
            body = {'filter': (filter or {}), 'limit': self._LIST_LIMIT, 'cursor': next_cursor, 'partition': f'{partition_num}/{partitions}', **(other_params or {})}
        ...
        while (len(next_cursors) > 0):
            tasks_summary = utils._concurrency.execute_tasks(get_partition, [(partition,) for partition in next_cursors], max_workers=partitions)
            if tasks_summary.exceptions:
    ...
    def _list(self, method, list_cls, resource_cls, resource_path=None, url_path=None, limit=None, filter=None, other_params=None, partitions=None, sort=None, headers=None, initial_cursor=None):
        if partitions:
            if (not is_unlimited(limit)):
                raise ValueError('When using partitions, limit should be `None`, `-1` or `inf`.')
            if (sort is not None):
                raise ValueError('When using sort, partitions is not supported.')
            return self._list_partitioned(partitions=partitions, method=method, list_cls=list_cls, resource_path=resource_path, filter=filter, other_params=other_params, headers=headers)
        resource_path = (resource_path or self._RESOURCE_PATH)
        items: List[T_CogniteResource] = []
        for resource_list in self._list_generator(list_cls=list_cls, resource_cls=resource_cls, resource_path=resource_path, url_path=url_path, method=method, limit=limit, chunk_size=self._LIST_LIMIT, filter=filter, sort=sort, other_params=other_params, headers=headers, initial_cursor=initial_cursor):
            items.extend(resource_list.data)
        return list_cls(items, cognite_client=self._cognite_client)

    def _list_partitioned(self, partitions, method, list_cls, resource_path=None, filter=None, other_params=None, headers=None):
        ...
        def get_partition(partition: int) -> List[Dict[(str, Any)]]:
            ...
            while True:
                if (method == 'POST'):
                    body = {'filter': (filter or {}), 'limit': self._LIST_LIMIT, 'cursor': next_cursor, 'partition': partition, **(other_params or {})}
                    res = self._post(url_path=((resource_path or self._RESOURCE_PATH) + '/list'), json=body, headers=headers)
                elif (method == 'GET'):
                    params = {**(filter or {}), 'limit': self._LIST_LIMIT, 'cursor': next_cursor, 'partition': partition, **(other_params or {})}
    ...
    def _aggregate(self, cls, resource_path=None, filter=None, aggregate=None, fields=None, keys=None, headers=None):
    ...
    @overload
    def _create_multiple(self, items, list_cls, resource_cls, resource_path=None, params=None, headers=None, extra_body_fields=None, limit=None):
        ...
    ...
    def _create_multiple(self, items, list_cls, resource_cls, resource_path=None, params=None, headers=None, extra_body_fields=None, limit=None):
        ...
        def str_format_element(el: T) -> Union[(str, Dict, T)]:
            if isinstance(el, CogniteResource):
                ...
                return dumped
            return el
        summary.raise_compound_exception_if_failed_tasks(task_unwrap_fn=(lambda task: task[1]['items']), task_list_element_unwrap_fn=unwrap_element, str_format_element_fn=str_format_element)
    ...
    def _delete_multiple(self, identifiers, wrap_ids, resource_path=None, params=None, headers=None, extra_body_fields=None):
        resource_path = (resource_path or self._RESOURCE_PATH)
        tasks = [{'url_path': (resource_path + '/delete'), 'json': {'items': (chunk.as_dicts() if wrap_ids else chunk.as_primitives()), **(extra_body_fields or {})}, 'params': params, 'headers': headers} for chunk in identifiers.chunked(self._DELETE_LIMIT)]
        summary = utils._concurrency.execute_tasks(self._post, tasks, max_workers=self._config.max_workers)
        summary.raise_compound_exception_if_failed_tasks(task_unwrap_fn=(lambda task: task['json']['items']), task_list_element_unwrap_fn=utils._auxiliary.unwrap_identifer)

    @overload
    def _update_multiple(self, items, list_cls, resource_cls, update_cls, resource_path=None, params=None, headers=None):
        ...
    ...
    def _update_multiple(self, items, list_cls, resource_cls, update_cls, resource_path=None, params=None, headers=None):
        ...
        tasks = [{'url_path': (resource_path + '/update'), 'json': {'items': chunk}, 'params': params, 'headers': headers} for chunk in patch_object_chunks]
        tasks_summary = utils._concurrency.execute_tasks(self._post, tasks, max_workers=self._config.max_workers)
        tasks_summary.raise_compound_exception_if_failed_tasks(task_unwrap_fn=(lambda task: task['json']['items']), task_list_element_unwrap_fn=(lambda el: utils._auxiliary.unwrap_identifer(el)))
    ...
    def _search(self, list_cls, search, filter, limit, resource_path=None, params=None, headers=None):
        utils._auxiliary.assert_type(filter, 'filter', [dict, CogniteFilter], allow_none=True)
        if isinstance(filter, CogniteFilter):
        ...
        resource_path = (resource_path or self._RESOURCE_PATH)
        res = self._post(url_path=(resource_path + '/search'), json={'search': search, 'filter': filter, 'limit': limit}, params=params, headers=headers)
    ...
    @classmethod
    def _raise_api_error(cls, res, payload):
        ...
        if res.history:
            for res_hist in res.history:
                log.debug(f'REDIRECT AFTER HTTP Error {res_hist.status_code} {res_hist.request.method} {res_hist.request.url}: {res_hist.content.decode()}')
    ...
    @staticmethod
    def _truncate(s, limit=500):
        if (len(s) > limit):
            return (s[:limit] + '...')
        return s

    @classmethod
    def _get_response_content_safe(cls, res):
        try:
