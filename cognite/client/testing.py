...
def _apply_async_mock_patches(mock_obj: MagicMock) -> None:
    """
    Recursively traverses a mock object, finds its spec, and replaces any
    async methods with signature-preserving AsyncMock instances.
    """
    spec_class = getattr(mock_obj, "_spec_class", None)
    if not spec_class:
        return

    for name, method in inspect.getmembers(spec_class, inspect.iscoroutinefunction):
        setattr(mock_obj, name, AsyncMock(spec_set=method, name=name))

    # Recurse into nested mocks that are part of the spec
    for name in dir(mock_obj):
        if name.startswith("_"):
            continue

        child_mock = getattr(mock_obj, name)
        if isinstance(child_mock, MagicMock) and hasattr(spec_class, name):
            _apply_async_mock_patches(child_mock)
...
class AsyncCogniteClientMock(MagicMock):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        # Recurse into 'self' and convert all async methods to AsyncMocks:
        _apply_async_mock_patches(self)
        return  # This means, no more code after this point!
...
def monkeypatch_cognite_client() -> Iterator[CogniteClientMock]:
    ...
    constructor = CogniteClient.__new__
    CogniteClient.__new__ = lambda *args, **kwargs: mock
    yield mock
    CogniteClient.__new__ = constructor
...
def monkeypatch_async_cognite_client() -> Iterator[AsyncCogniteClientMock]:
    ...
    constructor = AsyncCogniteClient.__new__
    AsyncCogniteClient.__new__ = lambda *args, **kwargs: mock
    yield mock
    AsyncCogniteClient.__new__ = constructor
