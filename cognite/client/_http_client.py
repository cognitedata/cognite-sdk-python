...
def get_global_requests_session():
    ...
    adapter = requests.adapters.HTTPAdapter(pool_maxsize=global_config.max_connection_pool_size, max_retries=urllib3.Retry(False))
...
class HTTPClientConfig():
    ...
    def __init__(self, status_codes_to_retry, backoff_factor, max_backoff_seconds, max_retries_total, max_retries_status, max_retries_read, max_retries_connect):
...
class _RetryTracker():
    ...
    def _max_backoff_and_jitter(self, t):
...
class HTTPClient():
    ...
    def __init__(self, config, session, retry_tracker_factory=_RetryTracker):
    ...
    def _do_request(self, method, url, **kwargs: Any):
        try:
            ...
            if self._any_exception_in_context_isinstance(e, (socket.timeout, urllib3.exceptions.ReadTimeoutError, requests.exceptions.ReadTimeout)):
                raise CogniteReadTimeout from e
            if self._any_exception_in_context_isinstance(e, (ConnectionError, urllib3.exceptions.ConnectionError, urllib3.exceptions.ConnectTimeoutError, requests.exceptions.ConnectionError)):
    ...
    @classmethod
    def _any_exception_in_context_isinstance(cls, exc, exc_types):
