...
from tests.utils import get_api_class_by_attribute
...
KNOWN_FILES_SKIP_LIST = {
    ...
    Path("cognite/client/_api/organization.py"),  # TODO?
}
MAYBE_IMPORTS = (
    "SortSpec: TypeAlias",
    ...
    for stmt in init_node.body:
        ...
        for t in stmt.targets:
            if (
                ...
                names.append(stmt.value.func.id)
                nested_apis.append(
                    # As opposed to the regular APIs, we only need to pass the async client here:
                    f"self.{t.attr} = Sync{stmt.value.func.id}(async_client)"
                )
    return names, nested_apis


def foolish_class_lookup(class_name: str) -> str:
    # Yes, YESSSS
    return {"ThreeD": "3D"}.get(class_name, class_name)
...
def generate_sync_client_code(
    ...
    generated_methods = []
    methods_by_name = {}
    for method_node in [m for m in class_def.body if isinstance(m, ast.AsyncFunctionDef)]:
    ...
    for name, method_nodes in methods_by_name.items():
        ...
        # 1. Generate overloads
        for overload_node in overloads:
            sync_def = f"@overload\n    def {name}({ast.unparse(overload_node.args)}) -> {ast.unparse(overload_node.returns)}: ..."
        ...
        # Assuming 'implementation' is an object with an 'args' attribute,
        # like a parsed function definition from the 'ast' module.
        call_parts = []

        # 1. Handle positional-only arguments (e.g., func(a, /))
        # These MUST be passed by position.
        ...
        # This must be passed with a single star.
        ...
        # This must be passed with a double star.
        ...
        if is_iterator:
            ...
            method_body = f"return SyncIterator(self.__async_client.{dotted_path}({', '.join(call_parts)}))"
        else:
            method_body = f"return self._run_sync(self.__async_client.{dotted_path}.{name}({', '.join(call_parts)}))"
    ...
    return (
        textwrap.dedent(
            SYNC_API_TEMPLATE.format(
                file_hash=file_hash,
                class_name=foolish_class_lookup(class_name),
...
def run_ruff(file_paths: list[Path]) -> None:
    ...
    base = f"poetry run ruff-{{}} --files {shlex.join(map(str, file_paths))}"
...
def create_sync_cognite_client(
    ...
    for api, attr in filter_base_apis_and_sort_alphabetically(dot_path_lookup):
        api = foolish_class_lookup(api)
        all_apis.append(f"self.{attr} = Sync{api}(async_client)\n")

        import_path = path_as_importable(
            SYNC_API_DIR / Path(file_path_lookup[api]).relative_to(ASYNC_API_DIR.resolve())
        ).replace(".__init__", "")
        all_imports.append(f"from {import_path} import Sync{api}")
