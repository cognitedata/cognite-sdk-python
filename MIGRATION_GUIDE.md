...
- All timestamp/datetime columns (from dataframes produced by a `to_pandas` call) now use millisecond precision, closely matching the API. This standardization comes after pandas
  v3 started inferring based on the input leading to arbitrary precision (sec/ms/µs/ns). Previosuly nanoseconds was the default for pandas v2.
...
- **ClientConfig**:
  - `max_workers` has functionally been removed (just throws a warning). Concurrency is now controlled via `global_config.concurrency_settings`.
    See the [Settings documentation](https://cognite-sdk-python.readthedocs-hosted.com/en/stable/settings.html#concurrency-settings) for details.
