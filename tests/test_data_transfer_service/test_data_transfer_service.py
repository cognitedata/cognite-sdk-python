...
from tests.conftest import TEST_TS_1_NAME, TEST_TS_REASONABLE_INTERVAL
...
class TestDataTransferService:
    ...
    def test_get_dataframes_w_column_mapping_and_global_aggregates(self, time_series_in_cdp):
        ts1 = TimeSeries(id=time_series_in_cdp[0], aggregates=["avg"], label="cavg")
        ts2 = TimeSeries(id=time_series_in_cdp[0], aggregates=["cv"], label="ccv")
        ts3 = TimeSeries(id=time_series_in_cdp[1], label="sinavg")
