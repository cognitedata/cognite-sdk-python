...
def asset_subtree():
    ...
    return get_asset_subtree(limit=1, api_key=os.getenv('COGNITE_API_KEY'), project='akerbp')


@pytest.fixture(scope='module')
def get_assets_response():
    from cognite.assets import get_assets
    return get_assets(limit=1, api_key=os.getenv('COGNITE_API_KEY'), project='akerbp')
