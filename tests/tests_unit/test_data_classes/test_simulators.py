...
class TestSimulatorModelRevisionDependency:
    ...
    def test_load_list(self, input_data: list[dict]) -> None:
        ...
        assert len(result) == 2
        file0 = result[0].file
        assert isinstance(file0, SimulatorModelDependencyFileId)
        assert file0.id == 1111
        assert result[0].arguments == {"fieldA": "valueA"}
        file1 = result[1].file
        assert isinstance(file1, SimulatorModelDependencyFileId)
        assert file1.id == 2222
