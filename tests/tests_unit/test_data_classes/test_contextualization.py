...
def mock_update_status_running(self: Any) -> str:
    self.status = "Running"
    return self.status


def mock_update_status_completed(self: Any) -> str:
...
class TestJobBundle:
    def test_DetectJobBundle_completed(self, mock_base_job_response: Any) -> None:
        _job_ids = [1, 2]
    ...
    def test_DetectJobBundle_one_running(self, mock_base_job_response: Any) -> None:
        completed_job_mock = mock_base_job_response
    ...
    def test_DetectJobBundle_failing(self, mock_base_job_response: Any) -> None:
        completed_job_mock = mock_base_job_response
        failed_job_mock = {
...
class TestDiagramDetectConfig:
    ...
    def test_overlapping_parameter_name(self, param_name: str) -> None:
        kwargs: dict[str, Any] = {param_name: True}
        with pytest.raises(ValueError, match="collides with a known parameter"):
