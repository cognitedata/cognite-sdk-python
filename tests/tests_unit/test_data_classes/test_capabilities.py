...
@pytest.fixture
def unknown_cap_with_extra_key() -> dict:
    return {
        "extra-key": {"urlNames": ["my-sandbox"]},
...
class TestCapabilities:
    ...
    def test_loads_to_known(self, access_control: dict[str, Any]) -> None:
    ...
    def test_load_dump__extra_top_level_keys(self, cap_with_extra_key: dict) -> None:
    ...
    def test_load_dump_unknown__extra_top_level_keys(self, unknown_cap_with_extra_key: dict) -> None:
        match = (
    ...
    def test_load_capability_misspelled_acl(self, unknown_acls_items: list[dict]) -> None:
    ...
    def test_load_data_model_instances__with_write_properties(
        ...
        has_write_allscope: bool,
        has_write_props_allscope: bool,
        has_write_in_same_scope: bool,
    ) -> None:
    ...
    def test_create_capability_forget_initializing_scope(self) -> None:
        # Ensure these do not raise. All other scopes require arguments and so will
        # raise appropriate errors if not initialized.
        ds1 = DataSetsAcl([DataSetsAcl.Action.Read], scope=DataSetsAcl.Scope.All)  # type: ignore[arg-type]
        ds2 = DataSetsAcl([DataSetsAcl.Action.Read], scope=DataSetsAcl.Scope.All())
        assert ds1.dump() == ds2.dump()

        ds1 = DataSetsAcl([DataSetsAcl.Action.Read], scope=AllScope)  # type: ignore[arg-type]
        ds2 = DataSetsAcl([DataSetsAcl.Action.Read], scope=AllScope())
        assert ds1.dump() == ds2.dump()

        grp1 = GroupsAcl([GroupsAcl.Action.Delete], scope=GroupsAcl.Scope.CurrentUser)  # type: ignore[arg-type]
        grp2 = GroupsAcl([GroupsAcl.Action.Delete], scope=GroupsAcl.Scope.CurrentUser())
        assert grp1.dump() == grp2.dump()

        grp1 = GroupsAcl([GroupsAcl.Action.Delete], scope=CurrentUserScope)  # type: ignore[arg-type]
    ...
    def test_create_capability_forget_initializing_scope__not_supported(self) -> None:
        ...
        with pytest.raises(ValueError, match="DataSetsAcl got an unknown scope"):
            DataSetsAcl(actions=[DataSetsAcl.Action.Read], scope=GroupsAcl.Scope.CurrentUser)  # type: ignore[arg-type]

        with pytest.raises(ValueError, match="ExperimentsAcl got an unknown scope"):
            ExperimentsAcl(actions=[ExperimentsAcl.Action.Use], scope=AllScope)  # type: ignore[arg-type]


@pytest.fixture
def proj_cap_allprojects_dct() -> dict:
...
@pytest.fixture
def proj_capabs_list(project_name: str) -> ProjectCapabilityList:
    return ProjectCapabilityList(
        [
            ...
            ProjectCapability(
                capability=EventsAcl([EventsAcl.Action.Write], scope=EventsAcl.Scope.DataSet(["1", 2])),  # type: ignore[list-item]
...
@pytest.fixture
def unknown_acls_items() -> list[dict]:
    return [
        # Unknown capability:
...
def mock_groups_resp(
    rsps: RequestsMock, cognite_client: CogniteClient, unknown_acls_items: list[dict]
...
class TestCogniteClientDoesntRaiseOnUnknownAcls:
    def test_groups_list(
        self, cognite_client: CogniteClient, mock_groups_resp: Any, unknown_acls_items: list[dict]
    ) -> None:
    ...
    def test_token_inspect(self, cognite_client: CogniteClient, mock_token_inspect_resp: Any) -> None:
...
class TestProjectCapabilityList:
    def test_project_scope_is_all_projects(self, proj_cap_allprojects_dct: dict) -> None:
        loaded = ProjectCapability.load(proj_cap_allprojects_dct.copy())
        assert type(loaded.project_scope) is AllProjectsScope

        loaded_list = ProjectCapabilityList.load([proj_cap_allprojects_dct])
        assert type(loaded_list[0].project_scope) is AllProjectsScope
...
class TestIAMCompareCapabilities:
    @pytest.mark.parametrize(
        ...
        [
            ...
            EventsAcl([EventsAcl.Action.Write], scope=EventsAcl.Scope.DataSet([1, "2"])),  # type: ignore[list-item]
    ...
    def test_has_capability(
        self,
        cognite_client: CogniteClient,
        proj_capabs_list: ProjectCapabilityList,
        project_name: str,
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            EventsAcl([EventsAcl.Action.Write], scope=EventsAcl.Scope.DataSet(["3"])),  # type: ignore[list-item]
    ...
    def test_partly_missing_capabilities(
        self, cognite_client: CogniteClient, proj_capabs_list: ProjectCapabilityList, project_name: str
    ) -> None:
        has = RawAcl([RawAcl.Action.Read], scope=RawAcl.Scope.Table({"my_db": ["my_table"]}))
        has_also = EventsAcl([EventsAcl.Action.Write], scope=EventsAcl.Scope.DataSet([1, "2"]))  # type: ignore[list-item]
    ...
    def test_raw_acl_database_scope_only(self, cognite_client: CogniteClient) -> None:
        # Would fail with: 'ValueError: No capabilities given' prior to 7.2.1 due to a bug in 'as_tuples'.
        has_all_scope = RawAcl([RawAcl.Action.Read], AllScope)  # type: ignore[arg-type]
    ...
    @pytest.mark.parametrize(
        ...
        [
            [],
            [RawAcl(actions=[RawAcl.Action.Read], scope=AllScope)],  # type: ignore[arg-type]
            [RawAcl(actions=[RawAcl.Action.Read], scope=TableScope({"db1": []}))],
            [RawAcl(actions=[RawAcl.Action.Read], scope=TableScope({"db1": {"tables": []}}))],
        ],
    )
    def test_raw_acl_database_scope(self, cognite_client: CogniteClient, extra_existing: list[Capability]) -> None:
        existing = [
    ...
    def test_unknown_existing_capability(self, cognite_client: CogniteClient) -> None:
    ...
    def test_legacy_capability(self, cognite_client: CogniteClient) -> None:
...
def test_idscopes_lower_case(dct: dict) -> None:
...
def test_idscopes_camel_case() -> None:
...
def test_show_example_usage(capability: type[Capability]) -> None:
