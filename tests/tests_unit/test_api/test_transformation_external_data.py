...
class TestTransformationExternalDataAPI:
    ...
    def test_list(self, cognite_client: CogniteClient) -> None:
        ...
        assert len(result) == 1
        assert isinstance(result[0], OneLakeExternalDataSource)
    ...
    def test_upsert_list(self, cognite_client: CogniteClient, httpx_mock: HTTPXMock, external_data_url: str) -> None:
        httpx_mock.add_response(
            method="POST",
            ...
            json={
                "items": [
                    {"externalId": "x", "format": "one_lake", "settings": {}},
                    {"externalId": "y", "format": "one_lake", "settings": {}},
                ]
            },
        )
        sources = [
            ...
            for external_id in ("x", "y")
        ]
        result = cognite_client.transformations.external_data_sources.upsert(sources)

        assert isinstance(result, ExternalDataSourceList)
        assert len(result) == 2
        request_body = jsgz_load(httpx_mock.get_requests()[-1].content)
    ...
    def test_delete(
        ...
        cognite_client.transformations.external_data_sources.delete("x", ignore_unknown_ids=True)
        ...
        assert request_body["ignoreUnknownIds"] is True

    def test_delete_ignore_unknown_ids_false(
        self,
        cognite_client: CogniteClient,
        mock_delete_response: HTTPXMock,
    ) -> None:
        ...
        request_body = jsgz_load(mock_delete_response.get_requests()[-1].content)
        assert request_body["ignoreUnknownIds"] is False
    ...
    def test_verify_usability_not_usable(
        ...
        httpx_mock.add_response(
            ...
            url=external_data_url + "/usability",
            status_code=200,
            json={"externalId": "x", "usableVersion": None},
        )
        result = cognite_client.transformations.external_data_sources.verify_usability("x")

        assert result.usable_version is None
        assert result.external_id == "x"

    @pytest.mark.usefixtures("mock_list_response")
    def test_as_write_round_trip_upsert(
        self, cognite_client: CogniteClient, httpx_mock: HTTPXMock, external_data_url: str
    ) -> None:
        httpx_mock.add_response(
            method="POST",
            url=external_data_url,
            status_code=201,
            json={"items": [{"externalId": "x", "format": "one_lake", "settings": {}}]},
        )
        listed = cognite_client.transformations.external_data_sources.list()
        write_source = listed[0].as_write(client_secret="new-secret")
        cognite_client.transformations.external_data_sources.upsert(write_source)

        request_body = jsgz_load(httpx_mock.get_requests()[-1].content)
        assert request_body["items"][0]["settings"]["credentials"]["clientSecret"] == "new-secret"
