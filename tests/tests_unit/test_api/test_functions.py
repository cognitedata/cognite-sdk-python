...
def mock_sessions_with_client_credentials(rsps, cognite_client_with_client_credentials_flow):
    url = full_url(cognite_client_with_client_credentials_flow,
                   "/sessions", api="iam.sessions")
    ...
    rsps.add(
        ...
        match=[post_body_matcher(
            {"items": [{"clientId": creds.client_id, "clientSecret": creds.client_secret}]})],
    )
...
def mock_functions_create_response(rsps, cognite_client):
    ...
    rsps.add(rsps.POST, files_byids_url, status=201,
             json={"items": [files_response_body]})
    functions_url = full_url(cognite_client, "/functions")
    rsps.add(rsps.POST, functions_url, status=201,
             json={"items": [EXAMPLE_FUNCTION]})
...
def mock_file_create_response(rsps, cognite_client):
    ...
    rsps.add(rsps.POST, full_url(cognite_client, "/files",
             api="files"), status=200, json=response_body)
...
class TestFunctionsAPI:
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("bad_absolute_import",
             "extra_root_folder/my_functions/handler.py", ModuleNotFoundError),
    ...
    def test_validate_folder(self, function_folder, function_path, exception):
        folder = os.path.join(os.path.dirname(__file__),
                              "function_test_resources", function_folder)
    ...
    def test_create_with_path(self, mock_functions_create_response, cognite_client):
        folder = os.path.join(os.path.dirname(__file__),
                              "function_test_resources", "function_code")
        res = cognite_client.functions.create(
            name="myfunction", folder=folder, function_path="handler.py")

        assert isinstance(res, Function)
        assert mock_functions_create_response.calls[3].response.json()[
    ...
    def test_create_with_file_id(self, mock_functions_create_response, cognite_client):
        ...
        assert isinstance(res, Function)
        assert mock_functions_create_response.calls[1].response.json()[
            "items"][0] == res.dump(camel_case=True)

    def test_create_with_function_handle(self, mock_functions_create_response, function_handle, cognite_client):
        res = cognite_client.functions.create(
            name="myfunction", function_handle=function_handle)
    ...
    def test_create_with_function_handle_with_illegal_name_raises(self, function_handle_illegal_name, cognite_client):
        with pytest.raises(TypeError):
            cognite_client.functions.create(
                name="myfunction", function_handle=function_handle_illegal_name)
    ...
    def test_create_with_function_handle_with_illegal_argument_raises(
        ...
        with pytest.raises(TypeError):
            cognite_client.functions.create(
                name="myfunction", function_handle=function_handle_illegal_argument)
    ...
    def test_create_with_handle_function_and_file_id_raises(
        ...
        with pytest.raises(TypeError):
            cognite_client.functions.create(
                name="myfunction", function_handle=function_handle, file_id=1234)
    ...
    def test_create_with_cpu_and_memory(self, mock_functions_create_response, cognite_client):
        res = cognite_client.functions.create(
            name="myfunction", file_id=1234, cpu=0.2, memory=1)
    ...
    def test_delete_multiple_id_and_multiple_external_id(self, mock_functions_delete_response, cognite_client):
        _ = cognite_client.functions.delete(
            id=[1, 2, 3], external_id=["func1", "func2"])
        assert {
    ...
    def test_list(self, mock_functions_filter_response, cognite_client):
        ...
        assert mock_functions_filter_response.calls[0].response.json()[
    ...
    def test_retrieve_by_id(self, mock_functions_retrieve_response, cognite_client):
        ...
        assert isinstance(res, Function)
        assert mock_functions_retrieve_response.calls[0].response.json()[
            "items"][0] == res.dump(camel_case=True)
    ...
    def test_retrieve_multiple_by_external_ids(self, mock_functions_retrieve_response, cognite_client):
        res = cognite_client.functions.retrieve_multiple(
            external_ids=["func1"])
        assert isinstance(res, FunctionList)
        assert mock_functions_retrieve_response.calls[0].response.json()[
            "items"] == res.dump(camel_case=True)
    ...
    def test_function_call_from_api_key_flow(self, mock_functions_call_responses, cognite_client_with_api_key):
        ...
        assert mock_functions_call_responses.calls[1].response.json(
        )["items"][0] == res.dump(camel_case=True)
    ...
    def test_function_call_by_external_id_from_api_key_flow(
        ...
        res = cognite_client_with_api_key.functions.call(
            external_id=f"func-no-{FUNCTION_ID}")
    ...
    def test_function_call_failed_from_api_key_flow(
        ...
        res = cognite_client_with_api_key.functions.call(id=FUNCTION_ID)
        assert isinstance(res, FunctionCall)
        assert mock_functions_call_failed_response.calls[0].response.json(
        ) == res.dump(camel_case=True)
    ...
    def test_function_call_timeout_from_api_key_flow(
        ...
        res = cognite_client_with_api_key.functions.call(id=FUNCTION_ID)
        assert isinstance(res, FunctionCall)
        assert mock_functions_call_timeout_response.calls[0].response.json(
        ) == res.dump(camel_case=True)
    ...
    def test_function_call_from_oidc_client_credentials_flow(
        ...
        res = cognite_client_with_client_credentials_flow.functions.call(
            id=FUNCTION_ID)
    ...
    def test_function_call_by_external_id_from_oidc_client_credentials_flow(
        ...
        assert isinstance(res, FunctionCall)
        expected = mock_functions_call_by_external_id_responses.calls[-1].response.json()[
            "items"][0]
    ...
    def test_function_call_with_failing_client_credentials_flow(self, cognite_client_with_client_credentials_flow):
        with pytest.raises(CogniteAPIError) as excinfo:
            cognite_client_with_client_credentials_flow.functions.call(
                id=FUNCTION_ID)
    ...
    def test_function_call_timeout_from_oidc_client_credentials_flow(
        ...
        res = cognite_client_with_client_credentials_flow.functions.call(
            id=FUNCTION_ID)
        assert isinstance(res, FunctionCall)
        assert mock_functions_call_timeout_response.calls[-1].response.json(
        ) == res.dump(camel_case=True)
    ...
    def test_function_call_from_oidc_token_exchange_flow(
        ...
        assert isinstance(res, FunctionCall)
        assert mock_functions_call_responses.calls[2].response.json(
        )["items"][0] == res.dump(camel_case=True)
    ...
    def test_function_call_by_external_id_from_oidc_token_exchange_flow(
        ...
        res = cognite_client_with_token.functions.call(
            external_id=f"func-no-{FUNCTION_ID}")
    ...
    def test_functions_limits_endpoint(self, mock_functions_limit_response, cognite_client):
        ...
        assert mock_functions_limit_response.calls[0].response.json(
        ) == res.dump(camel_case=True)
    ...
    def test_functions_status_endpoint(self, mock_functions_status_response, cognite_client):
        ...
        assert mock_functions_status_response.calls[0].response.json(
        ) == res.dump(camel_case=True)

        res = cognite_client.functions.status()
        assert isinstance(res, FunctionsStatus)
        assert mock_functions_status_response.calls[1].response.json(
        ) == res.dump(camel_case=True)
...
class TestRequirementsParser:
    ...
    def test_validate_requirements(self):
        parsed = _validate_and_parse_requirements(
            ["asyncio==3.4.3", "numpy==1.23.0", "pandas==1.4.3"])
        assert parsed == ["asyncio==3.4.3", "numpy==1.23.0", "pandas==1.4.3"]

    def test_validate_requirements_error(self):
        reqs = [["asyncio=3.4.3"], ["num py==1.23.0"],
    ...
    def test_extract_requirements_from_doc_string(self):
        ...
        assert _extract_requirements_from_doc_string(
            doc_string_mock) == expected
...
def mock_function_call_response_response(rsps, cognite_client):
    response_body = {"callId": CALL_ID,
                     "functionId": 1234, "response": {"key": "value"}}
    url = full_url(
        cognite_client, f"/functions/{FUNCTION_ID}/calls/{CALL_ID}/response")
    rsps.assert_all_requests_are_fired = False
    rsps.add(rsps.GET, url, status=200, json=response_body)
...
def mock_function_call_logs_response(rsps, cognite_client):
    ...
    url = full_url(
        cognite_client, f"/functions/{FUNCTION_ID}/calls/{CALL_ID}/logs")
    rsps.assert_all_requests_are_fired = False
    rsps.add(rsps.GET, url, status=200, json=response_body)
...
def mock_filter_function_schedules_response(rsps, cognite_client):
    ...
    rsps.add(rsps.POST, url, status=200, json={
             "items": [SCHEDULE_WITH_FUNCTION_EXTERNAL_ID]})
...
def mock_function_schedules_response_oidc_client_credentials(rsps, cognite_client):
    ...
    rsps.add(
        ...
        match=[post_body_matcher(
            {"items": [{"clientId": "aabbccdd", "clientSecret": "xxyyzz"}]})],
    )

    url = full_url(cognite_client, "/functions/schedules")
    rsps.add(rsps.POST, url, status=200, json={
             "items": [SCHEDULE_WITH_FUNCTION_ID_AND_SESSION]})
    yield rsps
...
def mock_schedule_get_data_response(rsps, cognite_client):
    ...
    url = full_url(
        cognite_client, f"/functions/schedules/{schedule_id}/input_data")
    rsps.add(rsps.GET, url, status=200, json={
             "id": schedule_id, "data": {"value": 2}})
...
class TestFunctionSchedulesAPI:
    def test_retrieve_schedules(self, mock_function_schedules_retrieve_response, cognite_client):
        res = cognite_client.functions.schedules.retrieve(
            id=SCHEDULE_WITH_FUNCTION_EXTERNAL_ID["id"])
    ...
    def test_list_schedules(self, mock_filter_function_schedules_response, cognite_client):
        ...
        expected = mock_filter_function_schedules_response.calls[0].response.json()[
            "items"]
        assert expected == res.dump(camel_case=True)
    ...
    def test_list_schedules_with_function_id_and_function_external_id_raises(self, cognite_client):
        with pytest.raises(AssertionError) as excinfo:
            cognite_client.functions.schedules.list(
                function_id=123, function_external_id="my-func")
        assert "Only function_id or function_external_id allowed when listing schedules." in str(
            excinfo.value)
    ...
    def test_create_schedules_with_function_external_id(self, mock_function_schedules_response, cognite_client):
        ...
        assert isinstance(res, FunctionSchedule)
        # make sure the created schedule has client ref
        assert res._cognite_client is cognite_client
        expected = mock_function_schedules_response.calls[0].response.json()[
            "items"][0]
        assert expected == res.dump(camel_case=True)
    ...
    def test_create_schedules_with_function_id_and_client_credentials(
        ...
        res = cognite_client.functions.schedules.create(
            ...
            cron_expression="*/5 * * * *",
            description="Hi",
            client_credentials={"client_id": "aabbccdd",
                                "client_secret": "xxyyzz"},
        )

        assert isinstance(res, FunctionSchedule)
        expected = mock_function_schedules_response_oidc_client_credentials.calls[1].response.json()[
            "items"][0]
        assert expected == res.dump(camel_case=True)
    ...
    def test_create_schedules_with_function_external_id_and_client_credentials_raises(self, cognite_client):
        with pytest.raises(ValueError, match="^When passing 'client_credentials', 'function_id' must be set$"):
            cognite_client.functions.schedules.create(
                ...
                description="Hi",
                client_credentials={
                    "client_id": "aabbccdd", "client_secret": "xxyyzz"},
            )
    ...
    def test_create_schedules_with_function_id_and_function_external_id_raises(self, cognite_client):
        ...
        assert "Exactly one of function_id and function_external_id must be specified" in str(
            excinfo.value)
    ...
    def test_schedule_get_data__function_has_data(self, mock_schedule_get_data_response, cognite_client):
        res = cognite_client.functions.schedules.get_input_data(
            id=8012683333564363)

        assert isinstance(res, dict)
        expected = mock_schedule_get_data_response.calls[0].response.json()[
            "data"]
        assert res == expected
...
class TestFunctionCallsAPI:
    def test_list_calls_and_filter(
        ...
        res = cognite_client.functions.retrieve(
            id=FUNCTION_ID).list_calls(**filter_kwargs, limit=-1)

        assert isinstance(res, FunctionCallList)
        assert mock_function_calls_filter_response.calls[1].response.json()[
            "items"] == res.dump(camel_case=True)
    ...
    def test_list_calls_by_function_id(self, mock_function_calls_filter_response, cognite_client):
        ...
        res = cognite_client.functions.calls.list(
            function_id=FUNCTION_ID, **filter_kwargs, limit=-1)
        assert isinstance(res, FunctionCallList)
        assert mock_function_calls_filter_response.calls[0].response.json()[
            "items"] == res.dump(camel_case=True)
    ...
    def test_list_calls_by_function_id_with_limits(
        ...
        res = cognite_client.functions.calls.list(
            function_id=FUNCTION_ID, limit=2)
        assert isinstance(res, FunctionCallList)
        assert len(res) == 2
    ...
    def test_list_calls_by_function_external_id(self, mock_function_calls_filter_response, cognite_client):
        res = cognite_client.functions.calls.list(
            function_external_id=f"func-no-{FUNCTION_ID}")
        assert isinstance(res, FunctionCallList)
        assert mock_function_calls_filter_response.calls[1].response.json()[
            "items"] == res.dump(camel_case=True)
    ...
    def test_list_calls_with_function_id_and_function_external_id_raises(self, cognite_client):
        with pytest.raises(AssertionError) as excinfo:
            cognite_client.functions.calls.list(
                function_id=123, function_external_id="my-function")
        assert "Exactly one of function_id and function_external_id must be specified" in str(
            excinfo.value)

    def test_retrieve_call_by_function_id(self, mock_function_calls_retrieve_response, cognite_client):
        res = cognite_client.functions.calls.retrieve(
            call_id=CALL_ID, function_id=FUNCTION_ID)
        assert isinstance(res, FunctionCall)
        assert mock_function_calls_retrieve_response.calls[0].response.json()[
            "items"][0] == res.dump(camel_case=True)

    @pytest.mark.usefixtures("mock_functions_retrieve_response")
    def test_retrieve_call_by_function_external_id(self, mock_function_calls_retrieve_response, cognite_client):
        res = cognite_client.functions.calls.retrieve(
            call_id=CALL_ID, function_external_id=f"func-no-{FUNCTION_ID}")
        assert isinstance(res, FunctionCall)
        assert mock_function_calls_retrieve_response.calls[1].response.json()[
            "items"][0] == res.dump(camel_case=True)
    ...
    def test_function_call_logs_by_function_id(self, mock_function_call_logs_response, cognite_client):
        res = cognite_client.functions.calls.get_logs(
            call_id=CALL_ID, function_id=FUNCTION_ID)
        assert isinstance(res, FunctionCallLog)
        assert mock_function_call_logs_response.calls[0].response.json()[
            "items"] == res.dump(camel_case=True)
    ...
    def test_function_call_logs_by_function_external_id(self, mock_function_call_logs_response, cognite_client):
        res = cognite_client.functions.calls.get_logs(
            call_id=CALL_ID, function_external_id=f"func-no-{FUNCTION_ID}")
        assert isinstance(res, FunctionCallLog)
        assert mock_function_call_logs_response.calls[1].response.json()[
            "items"] == res.dump(camel_case=True)
    ...
    def test_get_logs_on_retrieved_call_object(self, mock_function_call_logs_response, cognite_client):
        call = cognite_client.functions.calls.retrieve(
            call_id=CALL_ID, function_id=FUNCTION_ID)
        logs = call.get_logs()
        assert isinstance(logs, FunctionCallLog)
        assert mock_function_call_logs_response.calls[1].response.json()[
            "items"] == logs.dump(camel_case=True)
    ...
    def test_get_logs_on_created_call_object(
        ...
        call = cognite_client_with_client_credentials_flow.functions.call(
            id=FUNCTION_ID)
        logs = call.get_logs()
        assert isinstance(logs, FunctionCallLog)
        assert mock_function_call_logs_response.calls[-1].response.json()[
    ...
    def test_function_call_response_by_function_id(self, mock_function_call_response_response, cognite_client):
        res = cognite_client.functions.calls.get_response(
            call_id=CALL_ID, function_id=FUNCTION_ID)
        assert isinstance(res, dict)
        assert mock_function_call_response_response.calls[0].response.json()[
            "response"] == res
    ...
    def test_get_response_on_retrieved_call_object(self, mock_function_call_response_response, cognite_client):
        ...
        assert mock_function_call_response_response.calls[1].response.json()[
            "response"] == response
