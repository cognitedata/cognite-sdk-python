...
def mock_fit(httpx_mock: HTTPXMock, cognite_client: CogniteClient, async_client: AsyncCogniteClient) -> HTTPXMock:
    ...
    httpx_mock.add_response(
        method="POST",
        url=get_url(async_client.entity_matching) + cognite_client.entity_matching._RESOURCE_PATH + "/",
        status_code=200,
        json=response_body,
    )
    return httpx_mock
...
def mock_status_ok(httpx_mock: HTTPXMock, cognite_client: CogniteClient, async_client: AsyncCogniteClient) -> HTTPXMock:
    ...
    httpx_mock.add_response(
        method="GET",
        url=re.compile(f"{get_url(async_client.entity_matching)}{cognite_client.entity_matching._RESOURCE_PATH}/\\d+"),
        status_code=200,
        json=response_body,
    )
    return httpx_mock
...
def mock_retrieve(httpx_mock: HTTPXMock, cognite_client: CogniteClient, async_client: AsyncCogniteClient) -> HTTPXMock:
    ...
    httpx_mock.add_response(
        method="POST",
        url=re.compile(f"{get_url(async_client.entity_matching)}{cognite_client.entity_matching._RESOURCE_PATH}/byids"),
        status_code=200,
        json=response_body,
    )
    return httpx_mock
...
def mock_status_rules_ok(
    ...
    httpx_mock.add_response(
        method="GET",
        url=re.compile(
            f"{get_url(async_client.entity_matching)}{cognite_client.entity_matching._RESOURCE_PATH}/rules/\\d+"
        ),
        status_code=200,
        json=response_body,
    )
