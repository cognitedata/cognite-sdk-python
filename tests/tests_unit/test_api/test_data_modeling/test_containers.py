...
def mock_containers_response(
    httpx2_mock: Any, cognite_client: CogniteClient, async_client: AsyncCogniteClient
) -> HTTPXMock:
...
class TestContainersApi:
    ...
    def test_list_request_includes_used_for(
        self, httpx2_mock: Any, cognite_client: CogniteClient, async_client: AsyncCogniteClient
    ) -> None:
        base = get_url(async_client.data_modeling.containers) + "/models/containers"
        httpx2_mock.add_response(url=re.compile("^" + re.escape(base)), json={"items": []})
