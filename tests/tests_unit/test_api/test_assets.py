...
class TestAssets:
    ...
    def test_list_with_dataset_id_notnull(self, mock_assets_response):
        ASSETS_API.list(name="bla", data_set_id=[{"isNull": "false"}])
        assert ["isNull": "false"} == jsgz_load(mock_assets_response.calls[0]
        .request.body)["filter"][
            "dataSetId"
        ]
