...
@pytest.fixture
def mock_assets_response(httpx_mock, cognite_client):
...
def mock_get_subtree(mock_get_subtree_base, cognite_client):
    mock_get_subtree_base.add(
        ...
        get_url(cognite_client.assets) + "/assets/list",
        status_code=200,
        json={"items": []},
    )
...
class TestAssets:
    def test_retrieve_single(self, cognite_client, mock_assets_response):
        ...
        assert mock_assets_response.get_requests()[0].response.json()["items"][0] == res.dump(camel_case=True)
    ...
    def test_retrieve_multiple(self, cognite_client, mock_assets_response):
        ...
        assert mock_assets_response.get_requests()[0].response.json()["items"] == res.dump(camel_case=True)
    ...
    def test_iter_single(self, cognite_client, mock_assets_response):
        for asset in cognite_client.assets:
            assert mock_assets_response.get_requests()[0].response.json()["items"][0] == asset.dump(camel_case=True)

    def test_iter_chunk(self, cognite_client, mock_assets_response):
        for assets in cognite_client.assets(chunk_size=1):
            assert mock_assets_response.get_requests()[0].response.json()["items"] == assets.dump(camel_case=True)
