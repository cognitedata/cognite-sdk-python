...
def mock_get_datapoints(rsps, cognite_client):
    ...
    rsps.add_callback(
        rsps.POST,
        cognite_client.time_series.data.__synthetic._get_base_url_with_base_path() + "/timeseries/synthetic/query",
...
def mock_get_datapoints_empty(rsps, cognite_client):
    rsps.add(
        rsps.POST,
        re.compile(
            re.escape(cognite_client.time_series.data.__synthetic._get_base_url_with_base_path())
...
class TestSyntheticQuery:
    def test_query(self, cognite_client, mock_get_datapoints):
        dps_res = cognite_client.time_series.data.synthetic_query(
    ...
    def test_expression_builder(self, cognite_client):
        ...
        assert ("ts{externalId:'x'}", "a") == cognite_client.time_series.data.__synthetic._build_expression(
            symbols("a"), {"a": "x"}
        )
        assert (
            "ts{externalId:'x',aggregate:'average',granularity:'1m'}",
            "a",
        ) == cognite_client.time_series.data.__synthetic._build_expression(
        ...
        assert ("(1/ts{externalId:'a'})", "(1/a)") == cognite_client.time_series.data.__synthetic._build_expression(
            1 / symbols("a"), {"a": "a"}
        )
    ...
    def test_expression_builder_variables_missing(self, cognite_client):
        ...
        with pytest.raises(
            ...
            cognite_client.time_series.data.synthetic_query([symbols("a")], start=0, end="now")
    ...
    def test_expression_builder_unsupported_missing(self, cognite_client):
        ...
        with pytest.raises(ValueError, match="Unsupported sympy class cot"):
            cognite_client.time_series.data.synthetic_query(
