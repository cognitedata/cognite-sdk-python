...
class TestSchedules:
    ...
    def mock_post_schedule(self, rsps):
        rsps.add(
            rsps.POST,
            SCHEDULES_API._get_base_url_with_base_path() + "/analytics/models/schedules",
            status=201,
    ...
    def test_list_schedules(self, mock_get_schedules):
        res = SCHEDULES_API.list_schedules(limit=1)
        assert len(res) > 0
    ...
    def mock_get_schedule(self, rsps):
        rsps.add(
            rsps.GET,
            SCHEDULES_API._get_base_url_with_base_path() + "/analytics/models/schedules/1",
            json=self.schedule_response,
        )
        yield rsps

    def test_get_schedule(self, mock_get_schedule):
        res = SCHEDULES_API.get_schedule(id=1)
        assert isinstance(res, Schedule)
    ...
    def mock_put_deprecate(self, rsps):
        ...
        rsps.add(
            rsps.PUT,
            SCHEDULES_API._get_base_url_with_base_path() + "/analytics/models/schedules/1/deprecate",
            json=depr_schedule_response,
        )
        yield rsps

    def test_deprecate_schedule(self, mock_put_deprecate):
        res = SCHEDULES_API.deprecate_schedule(id=1)
        assert res.is_deprecated is True

    @pytest.fixture
    def mock_delete_schedule(self, rsps):
        rsps.add(rsps.DELETE, SCHEDULES_API._get_base_url_with_base_path() + "/analytics/models/schedules/1")

    def test_delete_schedule(self, mock_delete_schedule):
        res = SCHEDULES_API.delete_schedule(id=1)
        assert res is None
    ...
    def mock_get_log(self, rsps):
        ...
        rsps.add(
            rsps.GET,
            SCHEDULES_API._get_base_url_with_base_path() + "/analytics/models/schedules/1/log",
            json=schedule_log_response,
        )
        yield rsps

    def test_get_log(self, mock_get_log):
        res = SCHEDULES_API.get_log(id=1)
