...
@pytest.fixture
def mock_retrieve_latest(rsps: RequestsMock, cognite_client: CogniteClient) -> RequestsMock:
    def request_callback(request: Any) -> tuple[int, dict, str]:
...
@pytest.fixture
def mock_retrieve_latest_empty(rsps: RequestsMock, cognite_client: CogniteClient) -> RequestsMock:
    rsps.add(
        rsps.POST,
        cognite_client.time_series.data._get_base_url_with_base_path() + "/timeseries/data/latest",
        status=200,
...
@pytest.fixture
def mock_retrieve_latest_with_failure(rsps: RequestsMock, cognite_client: CogniteClient) -> RequestsMock:
    rsps.add(
        rsps.POST,
        cognite_client.time_series.data._get_base_url_with_base_path() + "/timeseries/data/latest",
        status=200,
...
class TestGetLatest:
    ...
    def test_retrieve_latest_empty(self, cognite_client: CogniteClient, mock_retrieve_latest_empty: Any) -> None:
        res = cognite_client.time_series.data.retrieve_latest(id=1)
        assert isinstance(res, Datapoints)
        assert 0 == len(res)

    def test_retrieve_latest_multiple_ts_empty(
        self, cognite_client: CogniteClient, mock_retrieve_latest_empty: Any
    ) -> None:
    ...
    def test_retrieve_latest_concurrent_fails(
        self, cognite_client: CogniteClient, mock_retrieve_latest_with_failure: Any, monkeypatch: MonkeyPatch
    ) -> None:
    ...
    def test_using_latest_datapoint_query__fails_wrong_ident(
        ...
        with pytest.raises(ValueError, match=err_msg):
            ...
            if pass_as == "external_id":
                ...
                raise ValueError("Invalid pass_as")
        ...
        with pytest.raises(ValueError, match=err_msg):
            ...
            if pass_as == "external_id":
                ...
                cognite_client.time_series.data.retrieve_latest(external_id=[valid, ldq, ldq, valid])
            elif pass_as == "id":
                assert isinstance(valid, int)
                cognite_client.time_series.data.retrieve_latest(id=[valid, ldq, ldq, valid])
            else:
                raise ValueError("Invalid pass_as")


@pytest.fixture
def mock_post_datapoints(rsps: RequestsMock, cognite_client: CogniteClient) -> RequestsMock:
    rsps.add(
        rsps.POST,
...
@pytest.fixture
def mock_post_datapoints_400(rsps: RequestsMock, cognite_client: CogniteClient) -> RequestsMock:
    rsps.add(
        rsps.POST,
        cognite_client.time_series.data._get_base_url_with_base_path() + "/timeseries/data",
        status=400,
...
class TestInsertDatapoints:
    def test_insert_tuples(self, cognite_client: CogniteClient, mock_post_datapoints: Any) -> None:
    ...
    def test_insert_dicts(self, cognite_client: CogniteClient, mock_post_datapoints: Any) -> None:
    ...
    def test_by_external_id(self, cognite_client: CogniteClient, mock_post_datapoints: Any) -> None:
        dps = [(i * 1e11, i) for i in range(1, 11)]
    ...
    def test_insert_datapoints_over_limit(
        self, cognite_client: CogniteClient, mock_post_datapoints: Any, monkeypatch: MonkeyPatch
    ) -> None:
    ...
    def test_insert_datapoints_no_data(self, cognite_client: CogniteClient) -> None:
    ...
    def test_insert_datapoints_in_multiple_time_series(
        self, cognite_client: CogniteClient, mock_post_datapoints: Any
    ) -> None:
    ...
    def test_insert_datapoints_ts_does_not_exist(
        self, cognite_client: CogniteClient, mock_post_datapoints_400: Any
    ) -> None:
...
@pytest.fixture
def mock_delete_datapoints(rsps: RequestsMock, cognite_client: CogniteClient) -> RequestsMock:
    rsps.add(
        rsps.POST,
...
class TestDeleteDatapoints:
    def test_delete_range(self, cognite_client: CogniteClient, mock_delete_datapoints: Any) -> None:
    ...
    def test_delete_range_invalid_id(
        self, cognite_client: CogniteClient, id: int | None, external_id: str | None, exception: type[Exception]
    ) -> None:
    ...
    def test_delete_ranges(self, cognite_client: CogniteClient, mock_delete_datapoints: Any) -> None:
...
class TestPandasIntegration:
    ...
    def test_insert_dataframe(self, cognite_client: CogniteClient, mock_post_datapoints: Any) -> None:
        import pandas as pd
    ...
    def test_insert_dataframe_external_ids(self, cognite_client: CogniteClient, mock_post_datapoints: Any) -> None:
        import pandas as pd
    ...
    def test_insert_dataframe_instance_ids(
        self, cognite_client: CogniteClient, mock_post_datapoints: Any, instance_ids: tuple
    ) -> None:
        import pandas as pd
    ...
    def test_insert_dataframe_with_dropna(self, cognite_client: CogniteClient, mock_post_datapoints: Any) -> None:
        import pandas as pd
    ...
    def test_insert_dataframe_single_dp(self, cognite_client: CogniteClient, mock_post_datapoints: Any) -> None:
        import pandas as pd
    ...
    def test_insert_dataframe_with_strings(
        self, cognite_client: CogniteClient, mock_post_datapoints: RequestsMock
    ) -> None:
        import pandas as pd
