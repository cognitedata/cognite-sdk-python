...
from cognite.client.data_classes.iam import GroupAttributes, ProjectSpec, TokenAttributes, TokenInspection
...
class TestGroups:
    ...
    def test_create_with_attributes(self, cognite_client, mock_groups_with_attributes):
        my_group = Group(
            ...
            attributes=GroupAttributes(token=TokenAttributes(app_ids=["app1", "app2"])),
        )
        res = cognite_client.iam.groups.create(my_group)
        assert isinstance(res, Group)
