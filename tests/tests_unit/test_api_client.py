...
class TestBasicRequests:
    ...
    async def test_requests_ok(
        ...
        assert (
            await api_client_with_token._config.credentials.authorization_header()[1]
    ...
    async def test_headers_correct_with_token(
        ...
        assert "api-key" not in headers
        assert await api_client_with_token._config.credentials.authorization_header()[1] == headers["Authorization"]
...
class TestStandardList:
    ...
    async def test_standard_list_generator(self, api_client_with_token: APIClient) -> None:
        total_resources = 0
        for resource in await api_client_with_token._list_generator(
            list_cls=SomeResourceList, resource_cls=SomeResource, resource_path=URL_PATH, method="GET"
        ):
    ...
    async def test_standard_list_generator_with_chunk_size(self, api_client_with_token: APIClient) -> None:
        total_resources = 0
        for resource_chunk in await api_client_with_token._list_generator(
    ...
    async def test_cognite_client_is_set_asfdsa(
        self, cognite_client: CogniteClient, api_client_with_token: APIClient, httpx_mock: HTTPXMock
    ) -> None:
        httpx_mock.add_response(
            method="POST",
...
class TestRetryableEndpoints:
    ...
    async def test_is_retryable_resource_api_endpoints(
    ...
    async def test_is_retryable(self, api_client_with_token: APIClient, method: str, path: str, expected: bool) -> None:
        assert expected is validate_url_and_return_retryability(method, path)

    @pytest.mark.parametrize(
        "method, path", [("POST", "htt://bla/bla"), ("BLOP", "http://localhost:8000/token/inspect")]
    )
    async def test_is_retryable_should_fail(self, api_client_with_token: APIClient, method: str, path: str) -> None:
