...
@pytest.fixture(scope="module", params=dps_params[:1])
def get_multitag_dps_response_obj(request):
    ...
    yield list(
        timeseries.get_multi_tag_datapoints(
...
class TestTimeseries:
    ...
    def test_post_multitag_datapoints(self):
        ...
        timeseries_with_too_many_datapoints = TimeseriesWithDatapoints(
            tagId="test", datapoints=[dto.Datapoint(x, x) for x in range(100001)]
        )
        timeseries_with_99999_datapoints = TimeseriesWithDatapoints(
            tagId="test", datapoints=[dto.Datapoint(x, x) for x in range(99999)]
        )
...
def test_split_TimeseriesWithDatapoints_if_over_limit():
    ...
    timeseries_with_datapoints_over_limit: TimeseriesWithDatapoints = TimeseriesWithDatapoints(
        tagId="test", datapoints=[Datapoint(x, x) for x in range(1000)]
    )
