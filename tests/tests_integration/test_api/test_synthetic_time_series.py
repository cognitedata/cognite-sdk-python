...
def post_spy(cognite_client):
    with mock.patch.object(
        cognite_client.time_series.data.__synthetic, "_post", wraps=cognite_client.time_series.data.__synthetic._post
    ) as _:
        yield
...
class TestSyntheticDatapointsAPI:
    def test_query(self, cognite_client, test_time_series, post_spy):
        ...
        dps = cognite_client.time_series.data.synthetic_query(
            expressions=query, start=datetime(2017, 1, 1), end="now", limit=23456
        )
        assert 23456 == len(dps)
        assert 3 == cognite_client.time_series.data.__synthetic._post.call_count
    ...
    def test_query_with_multiple_expressions(self, cognite_client, test_time_series, post_spy):
        ...
        assert 6 == cognite_client.time_series.data.__synthetic._post.call_count
    ...
    def test_expression_builder_time_series_vs_string(self, cognite_client, test_time_series):
        ...
        dps1 = cognite_client.time_series.data.synthetic_query(
        ...
        dps2 = cognite_client.time_series.data.synthetic_query(
