...
class TestInstancesAPI:
    ...
    def test_list_in_units(
        ...
        listed, info = cognite_client.data_modeling.instances.list(
            instance_type="node", include_typing=True, filter=is_node, sources=[source]
        )

        assert listed
        assert len(listed) == 1
        assert math.isclose(listed[0]["pressure"], 1.1 * 1e5)
        type_ = cast(Float64, info[unit_view.as_property_ref("pressure")].type)
