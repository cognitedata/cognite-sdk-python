...
from cognite.client.data_classes.raw import Database, RowWrite, Table
from cognite.client.exceptions import CogniteAPIError
from cognite.client.utils._text import random_string


@pytest.fixture(scope="session")
def new_database_with_table(cognite_client: CogniteClient) -> Iterator[tuple[Database, Table]]:
...
def db_database(cognite_client: CogniteClient) -> str:
    ...
    if db_name not in all_dbs.as_names():
        ...
        for i in range(1, 4):
            ...
            rows = [RowWrite(key=str(j), columns={f"c{k}": f"{j}_{k}" for k in range(10)}) for j in range(2000)]
            cognite_client.raw.rows.insert(db.name, table.name, rows)
    return db_name


class TestRawDatabasesAPI:
    def test_list_databases(self, cognite_client: CogniteClient) -> None:
        dbs = cognite_client.raw.databases.list()
        assert len(dbs) > 0


class TestRawTablesAPI:
    def test_list_tables(self, cognite_client: CogniteClient, db_database: str) -> None:
        tables = cognite_client.raw.tables.list(db_name=db_database)
        assert len(tables) == 3

    def test_create_and_delete_table(
        self, cognite_client: CogniteClient, new_database_with_table: tuple[Database, Table]
    ) -> None:
...
class TestRawRowsAPI:
    def test_list_rows(self, cognite_client: CogniteClient, db_database: str) -> None:
    ...
    def test_rows_with_parallel_cursors(self, cognite_client: CogniteClient) -> None:
    ...
    def test_list_rows_cols(self, cognite_client: CogniteClient, db_database: str) -> None:
    ...
    def test_iter_rows_cols(self, cognite_client: CogniteClient, db_database: str) -> None:
    ...
    def test_retrieve_row(self, cognite_client: CogniteClient, db_database: str) -> None:
    ...
    @pytest.mark.dsl
    def test_insert_and_retrieve_dataframe(
        self, cognite_client: CogniteClient, new_database_with_table: tuple[Database, Table]
    ) -> None:
        import pandas as pd
    ...
    @pytest.mark.dsl
    def test_insert_dataframe__index_has_duplicates(self, cognite_client: CogniteClient) -> None:
        import pandas as pd
