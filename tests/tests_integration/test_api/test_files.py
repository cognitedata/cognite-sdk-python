...
from cognite.client.data_classes.cdm.v1 import FileBaseApply
...
class TestFilesAPI:
    ...
    def test_create_retrieve_update_delete_with_instance_id(
        ...
        my_file = FileBaseApply(
            space=alpha_test_space.space,
            external_id="ts_python_sdk_instance_id_tests",
            is_uploaded=False,
            name="Create Retrieve Update Delete with Instance ID",
            description="This FileMetadata was created by the Python SDK",
        )
        update = FileMetadataUpdate(instance_id=my_file.as_id()).metadata.add({"a": "b"})

        try:
            created = cognite_client_alpha.data_modeling.instances.apply(my_file)
            assert len(created.nodes) == 1
            assert created.nodes[0].as_id() == my_file.as_id()

            retrieved = cognite_client_alpha.files.retrieve(instance_id=my_file.as_id())
            assert retrieved is not None
            assert retrieved.instance_id == my_file.as_id()
            ...
            retrieved = cognite_client_alpha.time_series.retrieve_multiple(instance_ids=[my_file.as_id()])
            assert retrieved.dump() == [updated.dump()]
        finally:
            cognite_client_alpha.data_modeling.instances.delete(nodes=my_file.as_id())
